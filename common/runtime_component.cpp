...
namespace TwilightDream::runtime_component
{
	...
	struct RuntimeTaskSlotSharedState final
	{
		std::size_t		 slot_index = 0;
		std::string		 display_name {};
		std::atomic<bool> stop_requested { false };
		std::atomic<std::size_t> assigned_worker_index { std::numeric_limits<std::size_t>::max() };
	};
	...
	public:
		...
		std::vector<std::shared_ptr<RuntimeTaskSlotSharedState>> slots {};
	...
	namespace
	{
		constexpr std::size_t g_runtime_invalid_worker_index = std::numeric_limits<std::size_t>::max();
		std::atomic<std::uint64_t> g_runtime_next_group_id { 1 };

		struct RuntimeThreadPoolWorkerState final
		{
			std::size_t				 worker_index = 0;
			std::atomic<std::uint64_t> generation { 1 };
			std::atomic<bool>		 alive { false };
			std::atomic<bool>		 busy { false };
			std::atomic<bool>		 retire_after_task { false };
			mutable std::mutex		 mutex {};
			std::string				 current_name {};
			std::thread::id			 thread_id {};
		};

		struct RuntimeQueuedTask final
		{
			std::shared_ptr<RuntimeTaskGroupSharedState> group {};
			std::shared_ptr<RuntimeTaskSlotSharedState>	slot {};
			std::function<void( RuntimeTaskContext& )> worker_fn {};
		};

		struct RuntimeThreadPoolState final
		{
			std::mutex										 mutex {};
			std::condition_variable							 cv {};
			std::size_t										 busy_worker_count = 0;
			std::deque<std::shared_ptr<RuntimeQueuedTask>>	 pending_tasks {};
			std::vector<std::shared_ptr<RuntimeThreadPoolWorkerState>> workers {};
		};

		static std::string runtime_idle_worker_name( std::size_t worker_index )
		{
			return "runtime-idle-t" + std::to_string( worker_index );
		}

		static std::string runtime_group_worker_name( const std::string& group_name, std::uint64_t group_id, std::size_t slot_index )
		{
			return group_name + "-g" + std::to_string( group_id ) + "-t" + std::to_string( slot_index );
		}

		static void set_current_thread_name_best_effort( const std::string& name )
		{
#if defined( _WIN32 )
			...
			( void )fn( GetCurrentThread(), wide_name.c_str() );
#else
			( void )name;
#endif
		}

		static RuntimeThreadPoolState& runtime_thread_pool_state()
		{
			static RuntimeThreadPoolState* pool = new RuntimeThreadPoolState();
			return *pool;
		}

		static void request_stop_all_for_group( const std::shared_ptr<RuntimeTaskGroupSharedState>& group )
		{
			if ( !group )
				return;

			{
				std::scoped_lock lk( group->mutex );
				group->stop_all_requested = true;
			}
			for ( const auto& slot : group->slots )
				slot->stop_requested.store( true, std::memory_order_relaxed );
			group->cv.notify_all();
		}

		static void capture_group_exception( const std::shared_ptr<RuntimeTaskGroupSharedState>& group, std::exception_ptr ex )
		{
			if ( !group || !ex )
				return;
			{
				std::scoped_lock lk( group->mutex );
				if ( !group->first_exception )
					group->first_exception = ex;
			}
			request_stop_all_for_group( group );
		}

		static void note_group_completion( const std::shared_ptr<RuntimeTaskGroupSharedState>& group )
		{
			if ( !group )
				return;
			{
				std::scoped_lock lk( group->mutex );
				if ( group->completed_count < group->slots.size() )
					++group->completed_count;
			}
			group->cv.notify_all();
		}

		static RuntimeWorkerSnapshot snapshot_worker_state(
			std::size_t											 slot_index,
			const std::shared_ptr<RuntimeThreadPoolWorkerState>& worker,
			bool													 stop_requested )
		{
			RuntimeWorkerSnapshot snapshot {};
			snapshot.slot_index = slot_index;
			snapshot.stop_requested = stop_requested;
			if ( !worker )
				return snapshot;

			snapshot.generation = worker->generation.load( std::memory_order_relaxed );
			snapshot.alive = worker->alive.load( std::memory_order_relaxed );
			snapshot.busy = worker->busy.load( std::memory_order_relaxed );
			{
				std::scoped_lock lk( worker->mutex );
				snapshot.current_name = worker->current_name;
				snapshot.thread_id = worker->thread_id;
			}
			return snapshot;
		}

		static void runtime_thread_pool_worker_main( RuntimeThreadPoolState* pool, const std::shared_ptr<RuntimeThreadPoolWorkerState>& worker );

		static void spawn_runtime_thread_pool_worker_locked( RuntimeThreadPoolState& pool, std::size_t worker_index, std::uint64_t generation )
		{
			if ( worker_index >= pool.workers.size() )
				pool.workers.resize( worker_index + 1 );

			auto worker = std::make_shared<RuntimeThreadPoolWorkerState>();
			worker->worker_index = worker_index;
			worker->generation.store( generation, std::memory_order_relaxed );
			{
				std::scoped_lock lk( worker->mutex );
				worker->current_name = runtime_idle_worker_name( worker_index );
			}
			pool.workers[ worker_index ] = worker;

			std::thread( [ &pool, worker ]() { runtime_thread_pool_worker_main( &pool, worker ); } ).detach();
		}

		static void ensure_runtime_thread_pool_capacity_locked( RuntimeThreadPoolState& pool, std::size_t target_worker_count )
		{
			if ( target_worker_count <= pool.workers.size() )
				return;

			const std::size_t first_new_index = pool.workers.size();
			pool.workers.resize( target_worker_count );
			for ( std::size_t index = first_new_index; index < target_worker_count; ++index )
				spawn_runtime_thread_pool_worker_locked( pool, index, 1 );
		}

		static void request_runtime_thread_pool_worker_retire( std::size_t worker_index )
		{
			auto& pool = runtime_thread_pool_state();
			std::shared_ptr<RuntimeThreadPoolWorkerState> worker {};
			{
				std::scoped_lock lk( pool.mutex );
				if ( worker_index < pool.workers.size() )
					worker = pool.workers[ worker_index ];
			}
			if ( worker )
				worker->retire_after_task.store( true, std::memory_order_relaxed );
		}

		static void runtime_thread_pool_worker_main( RuntimeThreadPoolState* pool, const std::shared_ptr<RuntimeThreadPoolWorkerState>& worker )
		{
			worker->alive.store( true, std::memory_order_relaxed );
			const std::string initial_name = runtime_idle_worker_name( worker->worker_index );
			{
				std::scoped_lock lk( worker->mutex );
				worker->thread_id = std::this_thread::get_id();
				worker->current_name = initial_name;
			}
			set_current_thread_name_best_effort( initial_name );

			for ( ;; )
			{
				std::shared_ptr<RuntimeQueuedTask> task {};
				{
					std::unique_lock lk( pool->mutex );
					pool->cv.wait( lk, [ & ]() { return !pool->pending_tasks.empty(); } );
					task = pool->pending_tasks.front();
					pool->pending_tasks.pop_front();
					++pool->busy_worker_count;
					worker->busy.store( true, std::memory_order_relaxed );
				}

				task->slot->assigned_worker_index.store( worker->worker_index, std::memory_order_relaxed );

				const std::string running_name = task->slot->display_name;
				{
					std::scoped_lock lk( worker->mutex );
					worker->thread_id = std::this_thread::get_id();
					worker->current_name = running_name;
				}
				set_current_thread_name_best_effort( running_name );

				RuntimeTaskContext context {};
				context.slot_index = task->slot->slot_index;
				context.current_name_ptr = &task->slot->display_name;
				context.stop_requested_ptr = &task->slot->stop_requested;

				bool should_run = !task->slot->stop_requested.load( std::memory_order_relaxed );
				if ( should_run )
				{
					std::scoped_lock group_lock( task->group->mutex );
					should_run = !task->group->stop_all_requested;
				}

				try
				{
					if ( should_run && task->worker_fn )
						task->worker_fn( context );
				}
				catch ( ... )
				{
					capture_group_exception( task->group, std::current_exception() );
				}

				const bool retire_after_task = worker->retire_after_task.exchange( false, std::memory_order_relaxed );
				{
					std::unique_lock lk( pool->mutex );
					if ( pool->busy_worker_count != 0 )
						--pool->busy_worker_count;
					worker->busy.store( false, std::memory_order_relaxed );

					if ( retire_after_task && worker->worker_index < pool->workers.size() && pool->workers[ worker->worker_index ].get() == worker.get() )
					{
						const std::uint64_t next_generation =
							worker->generation.load( std::memory_order_relaxed ) + 1;
						spawn_runtime_thread_pool_worker_locked( *pool, worker->worker_index, next_generation );
					}
				}

				if ( !retire_after_task )
				{
					const std::string idle_name = runtime_idle_worker_name( worker->worker_index );
					{
						std::scoped_lock lk( worker->mutex );
						worker->current_name = idle_name;
					}
					set_current_thread_name_best_effort( idle_name );
				}

				note_group_completion( task->group );
				pool->cv.notify_all();
	...
	bool RuntimeTaskGroupHandle::done() const
	{
		if ( !state_ )
			return true;
		std::scoped_lock lk( state_->mutex );
		return state_->completed_count >= state_->slots.size();
	}

	void RuntimeTaskGroupHandle::wait() const
	{
		if ( !state_ )
			return;

		std::unique_lock lk( state_->mutex );
		state_->cv.wait( lk, [ this ]() { return state_->completed_count >= state_->slots.size(); } );
		const std::exception_ptr ex = state_->first_exception;
		lk.unlock();
		if ( ex )
			std::rethrow_exception( ex );
	}

	void RuntimeTaskGroupHandle::request_stop_all() const
	{
		request_stop_all_for_group( state_ );
	}

	bool RuntimeTaskGroupHandle::request_stop_worker_by_name( const std::string& name ) const
	{
		...
		for ( const auto& slot : state_->slots )
		{
			...
			slot->stop_requested.store( true, std::memory_order_relaxed );
			const std::size_t worker_index = slot->assigned_worker_index.load( std::memory_order_relaxed );
			if ( worker_index != g_runtime_invalid_worker_index )
				request_runtime_thread_pool_worker_retire( worker_index );
	...
	std::vector<RuntimeWorkerSnapshot> RuntimeTaskGroupHandle::snapshot_workers() const
	{
		std::vector<RuntimeWorkerSnapshot> snapshots {};
		if ( !state_ )
			return snapshots;

		const std::vector<RuntimeWorkerSnapshot> pool_snapshots = snapshot_runtime_thread_pool_workers();
		snapshots.reserve( state_->slots.size() );
		for ( const auto& slot : state_->slots )
		{
			const std::size_t worker_index = slot->assigned_worker_index.load( std::memory_order_relaxed );
			if ( worker_index != g_runtime_invalid_worker_index && worker_index < pool_snapshots.size() )
			{
				RuntimeWorkerSnapshot snapshot = pool_snapshots[ worker_index ];
				snapshot.slot_index = slot->slot_index;
				snapshot.stop_requested = slot->stop_requested.load( std::memory_order_relaxed );
			...
			else
			{
				...
				snapshot.slot_index = slot->slot_index;
				snapshot.stop_requested = slot->stop_requested.load( std::memory_order_relaxed );
				snapshot.current_name = slot->display_name;
				snapshots.push_back( std::move( snapshot ) );
			}
		}
		return snapshots;
	...
	std::size_t RuntimeTaskGroupHandle::completed_count() const
	{
		if ( !state_ )
			return 0;
		std::scoped_lock lk( state_->mutex );
	...
	std::vector<RuntimeWorkerSnapshot> snapshot_runtime_thread_pool_workers()
	{
		...
		for ( std::size_t index = 0; index < workers.size(); ++index )
			snapshots.push_back( snapshot_worker_state( index, workers[ index ], false ) );
		return snapshots;
	}
	...
	RuntimeTaskGroupHandle start_named_worker_group(
		...
		std::function<void( RuntimeTaskContext& )> worker_fn )
	{
		const int threads = clamp_worker_thread_count( worker_thread_count );
		auto	  group = std::make_shared<RuntimeTaskGroupSharedState>();
		group->group_name = group_name.empty() ? std::string( "runtime-group" ) : group_name;
		group->group_id = g_runtime_next_group_id.fetch_add( 1, std::memory_order_relaxed );
		group->slots.reserve( std::size_t( threads ) );
		if ( !worker_fn )
			worker_fn = []( RuntimeTaskContext& ) {};

		auto& pool = runtime_thread_pool_state();
		{
			std::scoped_lock lk( pool.mutex );
			for ( int slot_index = 0; slot_index < threads; ++slot_index )
			{
				auto slot = std::make_shared<RuntimeTaskSlotSharedState>();
				slot->slot_index = std::size_t( slot_index );
				slot->display_name = runtime_group_worker_name( group->group_name, group->group_id, slot->slot_index );
				group->slots.push_back( slot );

				auto task = std::make_shared<RuntimeQueuedTask>();
				task->group = group;
				task->slot = slot;
				task->worker_fn = worker_fn;
				pool.pending_tasks.push_back( std::move( task ) );
			}

			const std::size_t target_worker_count = pool.busy_worker_count + pool.pending_tasks.size();
			ensure_runtime_thread_pool_capacity_locked( pool, target_worker_count );
		}

		pool.cv.notify_all();
		return RuntimeTaskGroupHandle( std::move( group ) );
	}
	...
	{
		RuntimeTaskGroupHandle handle = start_named_worker_group( group_name, worker_thread_count, std::move( worker_fn ) );
		try
		{
			if ( monitor_fn )
				monitor_fn();
	...
	NamedServiceThread::~NamedServiceThread()
	{
		stop();
	}

	void NamedServiceThread::start( const std::string& name, std::function<void()> fn )
	{
		stop();
		{
			std::scoped_lock lk( mutex_ );
		...
		worker_ = std::thread(
			[ this, fn = std::move( fn ), thread_name = name ]() mutable {
				alive_.store( true, std::memory_order_relaxed );
				set_current_thread_name_best_effort( thread_name );
				if ( fn )
					fn();
				alive_.store( false, std::memory_order_relaxed );
			} );
	...
	std::string NamedServiceThread::name() const
	{
		std::scoped_lock lk( mutex_ );
		return name_;
	}

	MemoryBallast::MemoryBallast( std::uint64_t headroom_bytes )
		: headroom_bytes_( headroom_bytes ),
		  service_thread_( std::make_unique<NamedServiceThread>() )
	{
	}
	...
	void MemoryBallast::start()
	{
		if ( !service_thread_ )
			service_thread_ = std::make_unique<NamedServiceThread>();
		if ( service_thread_->alive() )
			return;
		service_thread_->start( "runtime-memory-ballast", [ this ]() { run(); } );
	}
	...
	void MemoryBallast::run()
	{
		...
		while ( service_thread_ && !service_thread_->stop_requested() )
		{
			const SystemMemoryInfo mem = query_system_memory_info();
