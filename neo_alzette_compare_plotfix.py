...
The core pieces are:
    ...
    - Modular subtraction by constants modelled via an exact count‑based
      DP (maximising DP over output differences and reporting a
      ceil(-log2 DP) weight).
...
Note: the injection function is quadratic (due to dynamic‑mask AND/OR),
so for a fixed input difference Δ the XOR‑derivative
g_Δ(x) = f(x) ⊕ f(x ⊕ Δ) is affine.  Reachable output differences form an
affine space and each output occurs with probability 2^{-rank(L_Δ)} under
uniform x.  Therefore the injection contributes an additive weight equal
to this rank, which we compute via a small GF(2) linear‑basis routine.
"""
...
def cd_injection_from_B(b: int, rc0: int, rc1: int) -> tuple[int, int]:
    ...
    s_box_in_B  = (b  ^ RC[2]) ^ (~(b  & mask0))
    s_box_in_B2 = (b2) ^ (~(b2 | mask0))
    
    x0 = b2 ^ rc0 ^ mask0
    x1 = b ^ rc1 ^ mask0

    cross = b & rotr32(x0, 13)
    anti_shift = ((x1 >> 3) ^ (x1 >> 1)) & (b ^ rotr32(x0, 9))
...
def cd_injection_from_A(a: int, rc0: int, rc1: int) -> tuple[int, int]:
    ...
    s_box_in_A  = (a  ^ RC[7]) ^ (~(a  | mask1))
    s_box_in_A2 = (a2) ^ (~(a2 & mask1))
    
    x0 = a2 ^ rc0 ^ mask1
    x1 = a ^ rc1 ^ mask1

    cross = a & rotl32(x0, 19)
    anti_shift = ((x1 << 3) ^ (x1 << 1)) | (a ^ rotl32(x0, 9))
...
#     B ^= rotl(A, R1)
#
# Current V6 ordered instantiation:
#     R0 = 22
#     R1 = 13
#
# IMPORTANT:
# Keep these values consistent with the real cipher and every analysis /
# trace / search model. If one script uses different constants from the
# implementation, the experiment may silently analyze the wrong object.
#
# ------------------------------------------------------------------------
# Why these constants are not arbitrary
# ------------------------------------------------------------------------
# In earlier experiments, looser rotation choices produced bad behavior in
# automated ARX searches on 32-bit words. Some settings created alignment
# effects that made the injection layer easier to bypass, and the resulting
# best trails often contained too few active modular additions.
#
# Since modular addition is the main nonlinear component in this ARX design,
# "only a few active additions left" is treated as a structural red flag.
#
# For this bridge, the XOR-difference entering the injection side can contain
# a term like:
#
#     dB_prime = dB ^ rotl(dB, R0 + R1) ^ rotl(dA, R1)
#
# So the bridge-sum (R0 + R1) mod 32 matters structurally.
#
# If gcd(((R0 + R1) mod 32), 32) is large, there can exist large periodic
# subspaces where:
#
#     dB ^ rotl(dB, R0 + R1) == 0
#
# too easily. In practice, this means some repeating bit-pattern classes can
# partially cancel through the bridge, making the injection interface too
# transparent for single-branch XOR-difference propagation.
#
# To avoid this obvious escape hatch, we require:
#
#     gcd(((R0 + R1) % 32), 32) == 1
#
# Since 32 is a power of two, this is equivalent to:
#
#     ((R0 + R1) % 32) must be odd
#
...
#
# ------------------------------------------------------------------------
# Empirical screening rules behind the current pair
# ------------------------------------------------------------------------
# The underlying screened unordered pair is (13, 22), instantiated here as
# the ordered bridge constants (R0, R1) = (22, 13).
#
# The empirical filters were:
#
#   1) gcd(((r1 + r2) % 32), 32) == 1
#      -> avoid bridge sums aligned with large divisors of the word size
#
#   2) Exclude 8, 16, 24
#      -> quarter-turn / half-turn / three-quarter-turn positions in a
#         32-bit word
#      -> these repeatedly showed fragile alignment behavior in automated
#         searches and were more likely to make the injection layer easier
#         to bypass
#
#   3) Prefer moderate circular distance:
#          d(r1, r2) = min((r2 - r1) % 32, (r1 - r2) % 32)
#      with:
#          5 <= d(r1, r2) <= 11
#      -> avoid both overly local gaps and near-half-turn symmetry
#
#   4) Prefer one constant in [1, 15] and the other in [17, 31]
#      -> reduce same-region clustering and discourage fragile symmetric
#         layouts
#
...
#     (13 + 22) % 32 = 3
#
# These are empirical design filters derived from automated cryptanalytic
# experiments, not a proof of security.
#
# Bottom line:
# do not casually replace these constants with "nicer-looking" or more
# symmetric values unless the full search / trace / differential tooling is
# rerun.
# ========================================================================
CROSS_XOR_ROT_R0 = 22
CROSS_XOR_ROT_R1 = 13

def circular_distance(left_rotation: int, right_rotation: int, modulus: int = 32) -> int:
    return min(
        (right_rotation - left_rotation) % modulus,
        (left_rotation - right_rotation) % modulus,
    )

def validate_bridge_constants(r0: int, r1: int, modulus: int = 32) -> None:
    bridge_sum = (r0 + r1) % modulus
    distance = circular_distance(r0, r1, modulus)

    assert math.gcd(bridge_sum, modulus) == 1, (
        f"Invalid bridge constants: gcd((R0+R1) mod {modulus}, {modulus}) != 1; "
        f"got R0={r0}, R1={r1}, sum_mod={bridge_sum}"
    )

    assert r0 not in {8, 16, 24} and r1 not in {8, 16, 24}, (
        f"Invalid bridge constants: quarter/half-turn risk positions are forbidden; "
        f"got R0={r0}, R1={r1}"
    )

    assert 5 <= distance <= 11, (
        f"Invalid bridge constants: circular distance must be in [5, 11]; "
        f"got R0={r0}, R1={r1}, d={distance}"
    )

    assert ((1 <= r0 <= 15) and (17 <= r1 <= 31)) or ((1 <= r1 <= 15) and (17 <= r0 <= 31)), (
        f"Invalid bridge constants: constants should span different halves of the 32-bit word; "
        f"got R0={r0}, R1={r1}"
    )
...
def neoalzette_round_delta(delta_a: int, delta_b: int) -> tuple[int, int, float, float, float]:
    ...
    # Step 3: A ^= rotl(B,CROSS_XOR_ROT_R0)
    A ^= rotl32(B, CROSS_XOR_ROT_R0)

    # Step 4: B ^= rotl(A,CROSS_XOR_ROT_R1)
    ...
    # Step 3: B ^= rotl(A,CROSS_XOR_ROT_R0)
    B ^= rotl32(A, CROSS_XOR_ROT_R0)

    # Step 4: A ^= rotl(B,CROSS_XOR_ROT_R1)
...
def run_experiment(num_samples: int = 1000, seed: int = 0,
    ...
    try:
        ...
        plt.title(
            "Strict differential model (n=32): weight-difference distribution\n"
            f"NeoAlzette - Alzette, N={num_samples}, seed={seed}"
        )
...
def run_round_experiment(
    ...
    *,
    ...
    if reuse_samples:
        ...
        # Slow mode: fresh random pairs per r
        cache_alz: dict[tuple[int, int], tuple[int, int, float]] = {}
        cache_neo: dict[tuple[int, int], tuple[int, int, float, float, float]] = {}
...
def plot_round_trend(input_csv: str = "results_by_round.csv",
    ...
    try:
        ...
        title = "Strict differential model (n=32): mean weight vs rounds"
        meta = []
        if num_samples is not None:
