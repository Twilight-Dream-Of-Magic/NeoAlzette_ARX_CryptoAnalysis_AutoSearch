...
namespace
{
	...
	static void weight_shell_cache_rebuildable_cleanup_callback()
	{
		g_weight_shell_cache.clear_keep_enabled( "rebuildable_release" );
	}
	...
	static void print_usage_information( const char* executable_name )
	{
		std::cout << "Usage:\n"
				  ...
				  << "  --checkpoint-out PATH       Write binary checkpoint (default: auto_checkpoint_R... .ckpt).\n"
	...
	static std::uint64_t binom_u64( int n, int k )
	{
		if ( k < 0 || k > n )
			return 0;
		k = std::min( k, n - k );
		...
		for ( int i = 1; i <= k; ++i )
		{
			acc = acc * long double( n - k + i ) / long double( i );
	...
	static std::uint64_t estimate_weight_shell_bytes( int weight, int word_bits )
	{
		if ( word_bits < 2 )
			return 0;
		const int n = word_bits - 1;	// xdp_add weight counts bits [0..n-1]
	...
	static int compute_weight_shell_max_weight_from_budget( std::uint64_t budget_bytes, int word_bits )
	{
		const int max_w = std::clamp( word_bits - 1, 0, 31 );
		if ( max_w <= 0 )
			return 0;
		if ( budget_bytes == 0 )
			return std::min( 10, max_w );	// unknown budget: choose conservative default

		auto estimator = [ & ]( double threshold ) -> std::uint64_t {
			const int t = std::clamp( static_cast<int>( std::llround( threshold ) ), 0, max_w );
			const int w = max_w - t;	// larger threshold => smaller cached weight
			return estimate_weight_shell_bytes( w, word_bits );
		};
		...
		const int	 t = std::clamp( static_cast<int>( std::llround( threshold ) ), 0, max_w );
		const int	 w = std::clamp( max_w - t, 0, max_w );
		return w;
	}

static void configure_weight_shell_cache_for_run( DifferentialBestSearchConfiguration& cfg, std::uint64_t rebuildable_budget_bytes )
{
	if ( !cfg.enable_wpddt_cache )
	{
		cfg.wpddt_max_cached_weight = -1;
		g_weight_shell_cache.configure( false, 0 );
		return;
	}

	int max_weight = -1;
	if ( cfg.wpddt_max_cached_weight >= 0 )
	{
		max_weight = cfg.wpddt_max_cached_weight;
	}
	else
	{
			max_weight = compute_weight_shell_max_weight_from_budget( rebuildable_budget_bytes, 32 );
		}
		max_weight = std::clamp( max_weight, 0, 31 );
		cfg.wpddt_max_cached_weight = max_weight;
		g_weight_shell_cache.configure( true, max_weight );

		IosStateGuard g( std::cout );
		std::cout << "[Weight-Sliced pDDT] enabled=on  max_weight=" << max_weight;
	...
	static int run_auto_mode( const CommandLineOptions& command_line_options )
	{
		...
		configure_weight_shell_cache_for_run( base_configuration, rebuildable_pool().budget_bytes() );
		...
		std::cout << "  per_candidate: maximum_search_nodes=" << breadth_configuration.maximum_search_nodes << "  heuristic_branch_cap=" << heuristic_branch_cap( breadth_configuration ) << "  maximum_bit_flips=" << command_line_options.auto_breadth_max_bit_flips << "  state_memoization=" << ( breadth_configuration.enable_state_memoization ? "on" : "off" ) << "\n";
		if ( avail_bytes != 0 )
		{
...

		{
			...
			if ( command_line_options.checkpoint_out_was_provided || command_line_options.checkpoint_every_seconds_was_provided )
			{
				...
				const std::string path = command_line_options.checkpoint_out_was_provided ? command_line_options.checkpoint_out_path :
					default_binary_checkpoint_path( command_line_options.round_count, c.start_delta_a, c.start_delta_b );
			...
			try
			{
				( void )run_matsui_best_search_with_injection_internal( command_line_options.round_count, c.start_delta_a, c.start_delta_b, deep_configuration, true, deep_progress_sec,
																		...
																		seed_weight, seed_trail, checkpoint.out ? &checkpoint : nullptr, binary_checkpoint.enabled() ? &binary_checkpoint : nullptr );
			}
			catch ( const std::bad_alloc& )
			{
	...
	static int run_resume_mode( const CommandLineOptions& command_line_options )
	{
		...
		if ( !read_differential_checkpoint( command_line_options.resume_path, load, search_context.memoization ) )
		{
		...
		std::cout << "[Resume] round_count=" << load.configuration.round_count << "  visited_node_count=" << load.visited_node_count << "  best_total_weight=" << load.best_total_weight << "\n";
		{
			IosStateGuard g( std::cout );
		...
		DifferentialBestSearchConfiguration run_configuration = load.configuration;
		configure_weight_shell_cache_for_run( run_configuration, rebuildable_pool().budget_bytes() );

		search_context.configuration = run_configuration;
		search_context.start_difference_a = load.start_difference_a;
		search_context.start_difference_b = load.start_difference_b;
		search_context.visited_node_count = load.visited_node_count;
		...
		const auto now = std::chrono::steady_clock::now();
		search_context.run_start_time = ( load.elapsed_usec == 0 ) ? now : ( now - std::chrono::microseconds( load.elapsed_usec ) );
		if ( search_context.configuration.maximum_search_seconds != 0 )
		{
			...
			const std::uint64_t limit_usec =
				( search_context.configuration.maximum_search_seconds > ( std::numeric_limits<std::uint64_t>::max() / 1000000ull ) )
					? std::numeric_limits<std::uint64_t>::max()
					: ( search_context.configuration.maximum_search_seconds * 1000000ull );
		...
		RebuildableReserveGuard rebuildable_reserve( command_line_options.rebuildable_reserve_mib );
		PressureCallbackGuard	 pressure_guard( search_context.binary_checkpoint );

		if ( command_line_options.progress_every_seconds != 0 )
		{
			search_context.progress_every_seconds = command_line_options.progress_every_seconds;
			...
			search_context.progress_print_differences = false;
			{
	...
	static int run_single_mode( const CommandLineOptions& command_line_options )
	{
		...
		if ( command_line_options.frontend_mode == CommandLineOptions::FrontendMode::Strategy )
		{
			...
			std::cout << "  addition_weight_cap=" << run_configuration.addition_weight_cap << "  constant_subtraction_weight_cap=" << run_configuration.constant_subtraction_weight_cap << "  heuristic_branch_cap=" << heuristic_branch_cap( run_configuration ) << "  maximum_search_node=" << run_configuration.maximum_search_nodes << "  enable_state_memoization=" << ( run_configuration.enable_state_memoization ? "on" : "off" ) << "\n";
		...
		try
		{
			( void )run_matsui_best_search_with_injection_internal( command_line_options.round_count, command_line_options.initial_branch_a_difference, command_line_options.initial_branch_b_difference, run_configuration, true, command_line_options.progress_every_seconds, false, INFINITE_WEIGHT, nullptr, nullptr,
	...
	static int run_batch_mode( const CommandLineOptions& command_line_options )
	{
		...
		if ( command_line_options.frontend_mode == CommandLineOptions::FrontendMode::Strategy )
		{
			...
			std::cout << "  addition_weight_cap=" << run_configuration.addition_weight_cap << "  constant_subtraction_weight_cap=" << run_configuration.constant_subtraction_weight_cap << "  heuristic_branch_cap=" << heuristic_branch_cap( run_configuration ) << "  maximum_search_nodes=" << run_configuration.maximum_search_nodes << "  enable_state_memoization=" << ( run_configuration.enable_state_memoization ? "on" : "off" ) << "\n";
		...
		std::cout << "  per_job: addition_weight_cap=" << run_configuration.addition_weight_cap << " constant_subtraction_weight_cap=" << run_configuration.constant_subtraction_weight_cap << " heuristic_branch_cap=" << heuristic_branch_cap( run_configuration ) << " maximum_search_nodes=" << run_configuration.maximum_search_nodes << " enable_state_memoization=" << ( run_configuration.enable_state_memoization ? "on" : "off" ) << "\n";
		...
		DifferentialBestSearchConfiguration breadth_configuration = base_configuration;
		// Batch breadth: strict enumeration (no heuristic branching cap).
		heuristic_branch_cap( breadth_configuration ) = 0;
		...
		std::cout << "  breadth_maximum_search_nodes=" << breadth_configuration.maximum_search_nodes << "  breadth_heuristic_branch_cap=" << heuristic_branch_cap( breadth_configuration ) << " (strict)\n\n";
		...
		{
			IosStateGuard g( std::cout );
			std::cout << "\n[Batch][Breadth] done. elapsed_seconds=" << std::fixed << std::setprecision( 2 ) << elapsed_breadth << " total_nodes_visited=" << total_nodes_breadth.load() << " top_k_found=" << top_candidates.size() << "\n";
		}
		...
		// Stage 2: deep run on TOP-K jobs (parallel, one job per thread)
		// ---------------------------------------------------------------------
		DifferentialBestSearchConfiguration deep_configuration = run_configuration;
		...
		if ( expected != actual )
		{
			...
			if ( mismatch_index < min_size )
			{
				...
				std::cerr << "  first mismatch at index " << mismatch_index
						  << " expected_gamma=" << format_word32_hex( exp_entry.first ) << " expected_weight=" << exp_entry.second
						  << " actual_gamma=" << format_word32_hex( act_entry.first ) << " actual_weight=" << act_entry.second << "\n";
			}
			else
			{
	...
	{
		return {
			DiffToyCase { 0x00000015u, 0x0000002Bu, 0x00000000u, 6, 8 },
			DiffToyCase { 0x0000003Cu, 0x000000A1u, 0x0000005Au, 7, 8 },
			DiffToyCase { 0x0000012Du, 0x0000007Fu, 0x00000001u, 7, 9 },
			DiffToyCase { 0x00000155u, 0x000000AAu, 0x00000003u, 8, 10 },
			DiffToyCase { 0x00000345u, 0x000001A7u, 0x00000014u, 8, 12 }
		};
	}
	...
	static DiffToyCase make_random_diff_case( std::mt19937_64& rng )
	{
		...
		if ( c.alpha == 0 && c.beta == 0 )
			c.alpha = 1u;
		c.output_hint = pick_masked();

		const int min_cap = 4;
		int max_cap = std::min( 8, word_bits );
		if ( max_cap < min_cap )
			max_cap = min_cap;
	...
	static DifferentialBestSearchContext make_mod_add_test_context( bool enable_cache, int max_cached_weight )
	{
		DifferentialBestSearchContext ctx {};
		ctx.configuration.enable_wpddt_cache = enable_cache;
		ctx.configuration.wpddt_max_cached_weight = max_cached_weight;
		ctx.configuration.maximum_search_nodes = 0;
		return ctx;
	}
	...
	{
		g_weight_shell_cache.configure( false, 0 );
		g_weight_shell_cache.clear_keep_enabled();

		DifferentialBestSearchContext ctx = make_mod_add_test_context( false, 0 );
	...
	{
		DifferentialBestSearchContext ctx_cache = make_mod_add_test_context( true, tc.weight_cap );

		g_weight_shell_cache.configure( true, tc.weight_cap );
		g_weight_shell_cache.clear_keep_enabled();
		...
		if ( !ok_cache )
			return false;
		return compare_mod_add_sequences( tc, baseline, seq_cache, "baseline", "wpddt", "strict-consistency" );
	}

	static bool run_mod_add_checkpoint_resume_case( const DiffToyCase& tc, const ModAddSequence& baseline, std::mt19937_64& rng )
	{
		DifferentialBestSearchContext ctx = make_mod_add_test_context( true, tc.weight_cap );
		g_weight_shell_cache.configure( true, tc.weight_cap );
		g_weight_shell_cache.clear_keep_enabled();
	...
	static bool run_mod_add_memory_pressure_case( const DiffToyCase& tc, const ModAddSequence& baseline )
	{
		g_weight_shell_cache.clear_keep_enabled();
		g_weight_shell_cache.configure( true, tc.weight_cap );
		rebuildable_set_cleanup_fn( &weight_shell_cache_rebuildable_cleanup_callback );
		...
		if ( !checkpointed )
		{
			...
			print_diff_case( std::cerr, tc );
			std::cerr << " (did not enter cached shell)\n";
			rebuildable_set_cleanup_fn( nullptr );
			g_weight_shell_cache.clear_keep_enabled();
			g_weight_shell_cache.configure( false, 0 );
			return false;
		}
	...
	static bool run_differential_regression_tests( const CommandLineOptions& options )
	{
...
int main( int argument_count, char** argument_values )
{
	...
	if ( command_line_options.selftest )
	{
		...
		return run_differential_regression_tests( command_line_options ) ? 0 : 1;
	}
