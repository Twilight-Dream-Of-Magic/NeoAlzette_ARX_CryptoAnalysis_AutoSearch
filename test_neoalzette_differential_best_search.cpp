...
namespace
{
		...
		bool					  resume_mode ) noexcept
	{
		...
		if ( batch_mode )
		{
			...
			estimate = saturating_add_u64( estimate, saturating_mul_u64( std::uint64_t( std::max( 1, batch_threads ) ), 64ull * k_mib ) );
			estimate = saturating_add_u64(
				estimate,
				std::min<std::uint64_t>(
					1ull * k_gib,
		...
		bool					  resume_mode ) noexcept
	{
		return evaluate_memory_gate(
			...
			estimate_differential_must_live_bytes( command_line_options, auto_mode, batch_mode, resume_mode ),
	...
	static void print_usage_information( const char* executable_name )
	{
		std::cout << "Usage:\n"
				  ...
				  << "  Multi-job breadth->deep runs now live in test_neoalzette_differential_hull_wrapper.exe.\n"
				  << "  Use the wrapper for --batch-job-count / --batch-file, --thread-count, shared caches,\n"
	...
	static bool parse_command_line_auto_mode( int argument_count, char** argument_values, int start_index, CommandLineOptions& command_line_options )
	{
		...
		for ( ; argument_index < argument_count; ++argument_index )
		{
			...
			else if ( argument == "--pddt-max-weight" && argument_index + 1 < argument_count )
			{
				...
				if ( value < -1 || value > 31 )
					return false;
				command_line_options.search_configuration.weight_sliced_pddt_max_weight = value;
				command_line_options.search_configuration_was_provided = true;
			}
...
static void configure_weight_sliced_pddt_for_run( DifferentialBestSearchConfiguration& cfg, std::uint64_t rebuildable_budget_bytes )
{
	...
	{
		c.addition_weight_cap = std::clamp( c.addition_weight_cap, 0, 31 );
		c.constant_subtraction_weight_cap = std::clamp( c.constant_subtraction_weight_cap, 0, 32 );
	...
	static bool read_differential_auto_candidate( TwilightDream::auto_search_checkpoint::BinaryReader& r, DifferentialAutoCandidate& candidate )
	{
		...
		if ( !r.read_i32( candidate.entry_round1_weight ) ) return false;
	...
	static int run_auto_mode( const CommandLineOptions& command_line_options )
	{
		...
		const MemoryGateEvaluation memory_gate = evaluate_differential_memory_gate( command_line_options, mem, true, false, false );
		...
		BestSearchResult deep_result {};
		{
			...
			write_runtime_event_if_open( runtime_log, "auto_deep_selected", [ & ]( std::ostream& out ) {
				...
				out << "seeded_upper_bound_weight=" << ( ( c.best_weight >= INFINITE_WEIGHT ) ? -1 : c.best_weight ) << "\n";
	...
	static int run_differential_auto_resume_mode( const CommandLineOptions& command_line_options )
	{
		...
		const MemoryGateEvaluation memory_gate = evaluate_differential_memory_gate( command_line_options, mem, true, false, true );
		const bool strict_search_mode =
			( command_line_options.search_mode_was_provided ? command_line_options.search_mode : infer_search_mode_from_caps( command_line_options.search_configuration ) ) ==
			DifferentialSearchMode::Strict;
		...
		const int seed_weight = candidate.best_weight;
	...
	static int run_resume_mode( const CommandLineOptions& command_line_options )
	{
		...
		if ( checkpoint_kind != TwilightDream::auto_search_checkpoint::SearchKind::DifferentialBest )
		{
		...
		const MemoryGateEvaluation memory_gate = evaluate_differential_memory_gate( command_line_options, mem, false, false, true );
		const bool strict_search_mode =
			( command_line_options.search_mode_was_provided ? command_line_options.search_mode : infer_search_mode_from_caps( command_line_options.search_configuration ) ) ==
			DifferentialSearchMode::Strict;
	...
	static int run_single_mode( const CommandLineOptions& command_line_options )
	{
		...
		if ( avail_bytes != 0 && headroom_bytes > avail_bytes )
			headroom_bytes = avail_bytes;
		const MemoryGateEvaluation memory_gate = evaluate_differential_memory_gate( command_line_options, mem, false, false, false );
		const bool strict_search_mode =
			( command_line_options.search_mode_was_provided ? command_line_options.search_mode : infer_search_mode_from_caps( command_line_options.search_configuration ) ) ==
			DifferentialSearchMode::Strict;
...
int main( int argument_count, char** argument_values )
{
	...
	if ( command_line_options.batch_job_count > 0 || command_line_options.batch_job_file_was_provided )
	{
		std::cerr << "[Batch] Batch breadth→deep runs are implemented in test_neoalzette_differential_hull_wrapper (same flags: --batch-job-count / --batch-file, --seed, --thread-count, caches, --auto-breadth-maxnodes, --auto-deep-maxnodes, …).\n";
		return 1;
	}
