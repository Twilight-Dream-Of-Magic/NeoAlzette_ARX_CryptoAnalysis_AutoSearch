...
#include "auto_search_frame/differential_hull_batch_export.hpp"
...
namespace
{
	...
	struct CommandLineOptions
	{
		...
		int			  weight_sliced_pddt_max_weight = -1;
	...
	static bool parse_command_line( int argc, char** argv, CommandLineOptions& out )
	{
		...
		for ( int i = 1; i < argc; ++i )
		{
			...
			{
				if ( !parse_signed_integer_32( argv[ ++i ], out.weight_sliced_pddt_max_weight ) )
					return false;
				if ( out.weight_sliced_pddt_max_weight < -1 || out.weight_sliced_pddt_max_weight > 31 )
					return false;
				continue;
			}
		...
		const DifferentialBatchTopCandidate& candidate ) noexcept
	{
		...
		fp.mix_i32( candidate.entry_round1_weight );
	...
	static DifferentialStrictHullRuntimeOptions make_differential_runtime_options( const CommandLineOptions& options, DifferentialBestSearchRuntimeArtifacts* artifacts = nullptr, DifferentialCallbackHullAggregator* aggregator = nullptr )
	{
	...
	static DifferentialStrictHullRuntimeOptions make_differential_explicit_cap_runtime_options( const CommandLineOptions& options, DifferentialBestSearchRuntimeArtifacts* artifacts, int collect_weight_cap, DifferentialCallbackHullAggregator* aggregator = nullptr )
	{
		DifferentialStrictHullRuntimeOptions runtime_options {};
		runtime_options.collect_cap_mode = HullCollectCapMode::ExplicitCap;
		...
		DifferentialBestSearchRuntimeArtifacts* artifacts )
	{
		...
		return TwilightDream::hull_callback_aggregator::run_outer_growth_driver<
			...
			[]( int shell_weight, const DifferentialGrowthRuntimeState& state ) {
	...
	static bool run_differential_hull_aggregator_synthetic_self_test()
	{
		...
		const auto make_step =
			[]( int round_weight, std::uint32_t input_a, std::uint32_t input_b, std::uint32_t output_a, std::uint32_t output_b ) {
		...
		check(
			multi_source_aggregator.make_callback_for_source( TwilightDream::hull_callback_aggregator::DifferentialHullSourceContext { true, 0, 1, 0x10u, 0x20u } )( multi_source_view_a ),
			"multi-source aggregation should accept source #0" );
		check(
			multi_source_aggregator.make_callback_for_source( TwilightDream::hull_callback_aggregator::DifferentialHullSourceContext { true, 1, 1, 0x30u, 0x40u } )( multi_source_view_b ),
	...
	static int run_differential_batch_mode( const CommandLineOptions& options )
	{
		...
		if ( !resumed_batch_state_loaded && use_multi_trajectory_front_stage )
		{
			...
			else
			{
				...
				run_differential_batch_breadth_then_deep( jobs, worker_thread_count, orchestrator_config, orchestrator_result );
			}
		...
		else
		{
			...
			pipeline_options.runtime_options_template = make_differential_runtime_options( options, nullptr, nullptr );
		...
		std::cout << "\n[Batch][DifferentialHull] jobs=" << batch_summary.jobs.size()
				  ...
				  << "\n";
		print_differential_combined_source_hull_summary( pipeline_result, jobs.size(), selected_jobs.size() );
		if ( batch_runtime_log_enabled )
		{
			write_differential_batch_runtime_event(
				batch_runtime_log,
...
int main( int argc, char** argv )
{
	...
	const DifferentialStrictHullRuntimeResult runtime_result =
		run_differential_strict_hull_runtime(
			...
			options.delta_b,
			config,
			make_differential_runtime_options( options, &best_search_artifacts, &callback_aggregator ),
			options.verbose,
			false );
