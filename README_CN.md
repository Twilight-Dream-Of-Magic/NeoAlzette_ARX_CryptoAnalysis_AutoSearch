...
- **根工程研究/實驗程式**（`test_*.cpp`）：差分 trail trace、含注入模型的最佳 trail 搜尋、以及 hull wrapper。
- **獨立 PNB 子工程**：所有 PNB / neutral bits 工具已移到 `pnb_distinguisher/`。
...
- 因此，本 repo 的 `auto_search_frame` 不只是「借用術語」，而是把論文中的**低權首輪展開 / 中間輪 lookup + recombine / Matsui 式剪枝**，重寫成適合 NeoAlzette 與 checkpoint-resume 的顯式棧搜尋引擎。
...
- PNB 工具不再由根腳本建置；請改到 `pnb_distinguisher/` 內操作。
...
### `pnb_distinguisher/`（獨立 PNB 工作區）

所有 PNB 工具都已從根工程拆出，現在統一維護於 `pnb_distinguisher/`。

該子工程包含：

- 舊版 heatmap / event PNB 工具，
- 舊版 strict per-bit PNB 工具，
- 以及新的 `test_neoalzette_pnb_distinguisher`，用於 1-round conditional quotient-aware prototype。

請在 `pnb_distinguisher/` 內建置與執行這些 PNB 可執行檔，而不是從根工程操作。
