...
### 📚 **重要区分：Alzette vs NeoAlzette**

**❗ 重要澄清 ❗**：
- **Alzette**（2020论文原始设计）= 精妙简洁的三步流水线
- **NeoAlzette**（我们的项目实现）= 基于Alzette但大幅扩展的复杂实现

**艾瑞卡的洞察完全正确**：原始Alzette的厉害之处在于**三步流水线的精妙设计**！

### 🎨 **原始Alzette：三步流水线的工程艺术**

**论文中的Algorithm 1（真正的Alzette设计）**：
```
...
**艾瑞卡洞察的精妙之处**：
```
艾瑞卡的深度理解："如果这里两个变量单独的用模加模减就会导致相关性以及模加模减链极其快速叠加"

错误设计（直接模加）：
x = x + y;  // 简单粗暴
y = y + x;  // 进位链快速叠加
问题：carry(x,y) → carry(y+x,x) → carry(x',y') 模式收敛
结果：多轮后进位变得可预测，差分分析变trivial
...
Step 1后: x₁ = x₀ + (y₀ ≫ 31)
         ↳ x现在包含了y的信息，且通过模加引入了非线性

Step 2:   y₁ = y₀ ⊕ (x₁ ≫ 24)  
         ↳ y现在获得了x₁的复杂性（x₁已经非线性化）

关键：异或扩散不引入新的进位！
- 如果用模加：y = y + (x ≫ 24) 会产生新的carry依赖
- 用异或：y = y ⊕ (x ≫ 24) 提供完美的bit-level扩散，无副作用
```
...
**艾瑞卡的深度理解："减缓进位模加模减链快速叠加"**

**常量注入的深层作用**：
```
艾瑞卡的洞察："减缓进位模加模减链快速叠加"

原始Alzette的简洁解决方案：
Round 1: x₁ = x₀ + (y₀≫31), y₁ = y₀ ⊕ (x₁≫24), x₁ = x₁ ⊕ c
Round 2: x₂ = x₁ + (y₁≫17), y₂ = y₁ ⊕ (x₂≫17), x₂ = x₂ ⊕ c
...
...
**Alzette参数化设计的智慧**：
```
论文设计：Alzette_c (参数化by常量c)
- 每个不同的c产生不同的Alzette实例
- c通常从数学常数派生 (π, e, φ等的小数位)
- 安全性分析需要针对具体的c值进行
- 这种参数化让Alzette可以适配不同的应用需求
```
...
**原始Alzette的精确顺序哲学**：
```
Step 1: 非线性注入 → x ← x + (y ≫ r0)
作用：引入模加的非线性复杂性
原理：模加操作产生carry依赖，旋转打破bit对应
效果：x获得了来自y的复杂、非线性的信息

Step 2: 线性扩散 → y ← y ⊕ (x ≫ r1) 
作用：将x的非线性复杂性扩散到y
原理：XOR提供完美的bit-level扩散，无进位副作用
效果：y获得了x的复杂性，两个状态都变复杂
...
**艾瑞卡对NeoAlzette设计思路的深度理解**：
```
我们的NeoAlzette设计哲学：

性能排布的取舍：
- 原始Alzette：4对模加模减都用变量操作
- 我们的设计：只用2对，牺牲一些原始的简洁性

创新点：把中间常量从异或运算改为模加模减运算
- 原始：x ← x ⊕ c        (异或常量)
- 我们：A -= RC[1]       (模减常量 = 模加模减运算)

补偿策略：用更强的线性函数
- 添加L1_forward, L2_forward线性扩散层
- 复杂的cd_from_B跨分支注入
- 确保扩散性够强，产生连锁反应效果
