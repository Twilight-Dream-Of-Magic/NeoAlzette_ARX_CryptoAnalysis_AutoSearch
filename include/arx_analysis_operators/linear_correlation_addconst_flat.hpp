#pragma once
/**
 * @file linear_correlation_addconst_flat.hpp
 ...
 * - By default, uses `__int128` when available (GCC/Clang).
 * - Otherwise, falls back to the built-in fixed-width `FixedInt256`.
 * - Define `TWILIGHTDREAM_ARX_FORCE_INT256` to force the portable backend.
 *
...
namespace TwilightDream::arx_operators
{
	...
	static inline int popcnt_u64( std::uint64_t x ) noexcept
	{
		return static_cast<int>( std::popcount( x ) );
	}

	static inline int ctz_u64( std::uint64_t x ) noexcept
	{
		// Defined for x==0: returns 64.
	...
	// ============================================================================
	// Fixed 256-bit signed integer (two's complement, header-only)
	// ============================================================================

	struct FixedInt256
	{
		// Little-endian limbs (limb[0] = low 64 bits).
		std::uint64_t limb[4] { 0, 0, 0, 0 };

		constexpr FixedInt256() noexcept = default;

		// NOTE: Intentionally implicit to allow `FixedInt256{1}` / `{0}`.
		constexpr FixedInt256( std::int64_t v ) noexcept
		{
			limb[0] = static_cast<std::uint64_t>( v );
		...
		[[nodiscard]] static inline FixedInt256 neg_twos_complement( FixedInt256 x ) noexcept
		{
			x.limb[0] = ~x.limb[0];
			x.limb[1] = ~x.limb[1];
			x.limb[2] = ~x.limb[2];
			...
			for ( int i = 0; i < 4; ++i )
			{
				const std::uint64_t before = x.limb[i];
				x.limb[i] = before + carry;
				carry = ( x.limb[i] < before ) ? 1 : 0;
			}
			return x;
		}

		[[nodiscard]] friend inline FixedInt256 operator-( FixedInt256 x ) noexcept
		{
			return neg_twos_complement( x );
		}

		[[nodiscard]] friend inline FixedInt256 operator+( FixedInt256 a, const FixedInt256& b ) noexcept
		{
			std::uint64_t carry = 0;
			for ( int i = 0; i < 4; ++i )
			{
				const std::uint64_t ai = a.limb[i];
				std::uint64_t		  s = ai + b.limb[i];
				const std::uint64_t	  c1 = ( s < ai ) ? 1 : 0;
				const std::uint64_t	  before = s;
				s += carry;
				const std::uint64_t c2 = ( s < before ) ? 1 : 0;
				a.limb[i] = s;
				carry = ( c1 | c2 );
			}
			return a;
		...
		friend inline FixedInt256& operator+=( FixedInt256& a, const FixedInt256& b ) noexcept
		{
			a = a + b;
			return a;
		}
		...
		[[nodiscard]] friend inline bool operator==( const FixedInt256& a, const FixedInt256& b ) noexcept
		{
			return a.limb[0] == b.limb[0] && a.limb[1] == b.limb[1] && a.limb[2] == b.limb[2] && a.limb[3] == b.limb[3];
		}
		...
		[[nodiscard]] inline FixedInt256 shl( int k ) const noexcept
		{
			if ( k <= 0 )
				return *this;
			if ( k >= 256 )
				return FixedInt256 { 0 };

			FixedInt256 r { 0 };
			const int word = ( k >> 6 );	  // /64
			const int bit = ( k & 63 );	  // %64

			for ( int di = 3; di >= 0; --di )
			{
				const int si = di - word;
				if ( si < 0 )
				{
					r.limb[di] = 0;
					continue;
				...
				std::uint64_t out = ( bit == 0 ) ? limb[si] : ( limb[si] << bit );
				if ( bit != 0 && ( si - 1 ) >= 0 )
					out |= ( limb[si - 1] >> ( 64 - bit ) );
		...
		[[nodiscard]] inline int bit_width_abs() const noexcept
		{
			const bool neg = is_negative();
			const FixedInt256 mag = neg ? -( *this ) : *this;

			for ( int i = 3; i >= 0; --i )
			{
				const std::uint64_t w = mag.limb[i];
				if ( w != 0 )
					return ( i * 64 ) + int( std::bit_width( w ) );
			}
			return 0;
		}
		...
		[[nodiscard]] inline long double to_long_double() const noexcept
		{
			...
			long double v = 0.0L;
			for ( int i = 3; i >= 0; --i )
				v = std::scalbn( v, 64 ) + static_cast<long double>( mag.limb[i] );
		...
		[[nodiscard]] inline double to_double() const noexcept
		{
			...
			double v = 0.0;
			for ( int i = 3; i >= 0; --i )
				v = std::scalbn( v, 64 ) + static_cast<double>( mag.limb[i] );
	...
	namespace detail
	{
#if defined( __SIZEOF_INT128__ ) && !defined( TWILIGHTDREAM_ARX_FORCE_INT256 )
		using WideInt = __int128;
#else
		using WideInt = FixedInt256;
#endif

#if defined( __SIZEOF_INT128__ )
		// Shift-left for signed __int128 without UB on negative values.
		// We rely on the fact that our values stay well within the signed __int128 range for n<=64.
		static inline __int128 shl_wide( __int128 x, int k ) noexcept
		{
			if ( k <= 0 )
				return x;
			const unsigned __int128 ux = static_cast<unsigned __int128>( x );
			const unsigned __int128 uy = ( ux << k );
			return static_cast<__int128>( uy );
		}

		[[nodiscard]] static inline bool is_zero_wide( __int128 x ) noexcept
		{
			return x == 0;
		}

		[[nodiscard]] static inline long double to_long_double_wide( __int128 x ) noexcept
		{
			return static_cast<long double>( x );
		}

		[[nodiscard]] static inline double to_double_wide( __int128 x ) noexcept
		{
			return static_cast<double>( x );
		}

		// floor(log2(|x|)) as a 0-based bit index, for x != 0.
		[[nodiscard]] static inline int abs_msb_index_wide( __int128 x ) noexcept
		{
			const unsigned __int128 a =
				( x < 0 ) ? static_cast<unsigned __int128>( -x ) : static_cast<unsigned __int128>( x );

			const std::uint64_t hi = std::uint64_t( a >> 64 );
			const std::uint64_t lo = std::uint64_t( a );
			if ( hi != 0 )
				return 64 + ( int( std::bit_width( hi ) ) - 1 );
			return ( int( std::bit_width( lo ) ) - 1 );
		}
#endif

		static inline FixedInt256 shl_wide( const FixedInt256& x, int k ) noexcept
		{
			return x.shl( k );
		}

		[[nodiscard]] static inline bool is_zero_wide( const FixedInt256& x ) noexcept
		{
			return x.is_zero();
		}

		[[nodiscard]] static inline long double to_long_double_wide( const FixedInt256& x ) noexcept
		{
			return x.to_long_double();
		}

		[[nodiscard]] static inline double to_double_wide( const FixedInt256& x ) noexcept
		{
			return x.to_double();
		}

		// floor(log2(|x|)) as a 0-based bit index, for x != 0.
		[[nodiscard]] static inline int abs_msb_index_wide( const FixedInt256& x ) noexcept
		{
	...
	namespace detail
	{
		...
		) noexcept
		{
			...
			while ( true )
			{
				const int bpos = ( beta_rel != 0 ) ? ctz_u64( beta_rel ) : run_len;
				const int len = bpos - seg;
				if ( len > 0 )
				{
					const std::uint64_t seg_mask_rel =
						( len >= 64 ) ? ~std::uint64_t( 0 ) : ( ( std::uint64_t( 1 ) << len ) - 1 );
					const int k1 = popcnt_u64( ( t_rel >> seg ) & seg_mask_rel );
		...
		) noexcept
		{
			...
			while ( true )
			{
				...
				if ( len > 0 )
				{
					...
					const int kS = popcnt_u64( active_seg );
					if ( kS > 0 )
					{
						const int k1 = popcnt_u64( active_seg & ( ( t_rel >> seg ) & seg_mask_rel ) );
		...
		) noexcept
		{
			...
			while ( active_mask != 0 )
			{
				const int bit_index = ctz_u64( active_mask );
	...
	) noexcept
	{
		...
		const int parity_bc = popcnt_u64( beta & run_table.constant ) & 1;
	...
	namespace detail
	{
		...
		) noexcept
		{
			...
			while ( active_runs != 0 )
			{
				...
				const int run_index = ctz_u64( active_run_bit );
				const detail::FlatRunInfo& run = run_table.runs[run_index];
				...
				const int active_count = popcnt_u64( active_rel );
				const int beta_count = popcnt_u64( beta_rel );
				const int event_count = active_count + beta_count;

				if ( beta_count == 0 )
				{
					const int k1 = popcnt_u64( active_rel & t_rel );
	...
	[[nodiscard]] static inline int linear_correlation_add_const_exact_flat_weight_ceil_int(
		std::uint64_t alpha,
		std::uint64_t constant,
		std::uint64_t beta,
		int			  n
	...
	[[nodiscard]] static inline int corr_add_const_exact_flat_weight_ceil_int(
		std::uint64_t alpha,
		std::uint64_t beta,
		std::uint64_t constant,
		int			  n
	...
	) noexcept
	{
		...
		const int wt_beta = popcnt_u64( beta );
	...
	[[nodiscard]] static inline CascadeReport corr_add_const_cascade( std::span<const CascadeRound> rounds ) noexcept
	{
		...
		for ( const CascadeRound& rd : rounds )
		{
			...
			if ( rd.lift )
			{
				...
				delta += 2.0L * static_cast<long double>( popcnt_u64( liftm.beta_res & mask_n_u64( nrd ) ) ) * std::ldexp( 1.0L, -Luse );
			}
			else
			{
				const WindowedCorrelationReport r = linear_correlation_add_const_flat_bin_report( rd.alpha, rd.constant, rd.beta, nrd, Lrd );
				chat *= r.corr_hat.as_long_double();
				delta += 2.0L * static_cast<long double>( popcnt_u64( rd.beta & mask_n_u64( nrd ) ) ) * std::ldexp( 1.0L, -Lrd );
			}
		}
