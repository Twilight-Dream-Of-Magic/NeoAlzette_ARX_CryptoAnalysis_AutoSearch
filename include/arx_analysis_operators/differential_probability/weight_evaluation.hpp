/**
 * @file differential_probability/weight_evaluation.hpp
 ...
 * 3) API 约定
 * ----------------------------------------------------------------------------
 * - `xdp_add_lm2001` / `xdp_add_lm2001_n`：返回权重 `SearchWeight`；若不可能则返回 `INFINITE_WEIGHT`。
...
namespace TwilightDream
{
	namespace arx_operators
	{
		 ...
		 * @return 权重 `SearchWeight`；若差分不可能则返回 `INFINITE_WEIGHT`
		 */
		inline SearchWeight xdp_add_lm2001( std::uint32_t alpha, std::uint32_t beta, std::uint32_t gamma ) noexcept
		{
			...
			{
				return INFINITE_WEIGHT;	// Impossible differential
			}
			...
			const SearchWeight weight = static_cast<SearchWeight>( std::popcount( masked_bad ) );
		...
		{
			const SearchWeight weight = xdp_add_lm2001( alpha, beta, gamma );
		...
		{
			return xdp_add_lm2001( alpha, beta, gamma ) < INFINITE_WEIGHT;
		}
		 ...
		 */
		inline SearchWeight xdp_add_lm2001_n( std::uint32_t alpha, std::uint32_t beta, std::uint32_t gamma, int n ) noexcept
		{
