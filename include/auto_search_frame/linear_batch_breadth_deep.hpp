...
namespace TwilightDream::hull_callback_aggregator
{
	...
	// Runs stage-1 breadth over all jobs (parallel) keeping top-K = min(jobs, threads), then stage-2 deep (parallel).
		...
		LinearBatchBreadthDeepOrchestratorResult&		 out )
	{
		...
		const std::size_t deep_top_k = std::min<std::size_t>( jobs.size(), std::size_t( threads ) );
		...
		run_worker_threads_with_monitor( threads, [ & ]( int t ) { breadth_worker( t ); }, breadth_progress_monitor );
		...
		LinearBatchBreadthDeepOrchestratorResult& out )
	{
		...
		const int threads = std::max( 1, worker_thread_count );
		const std::size_t deep_top_k = std::min<std::size_t>( state.jobs.size(), std::size_t( threads ) );
		if ( state.completed_job_flags.size() < state.jobs.size() )
			state.completed_job_flags.resize( state.jobs.size(), 0u );
		linear_sort_batch_top_candidates( state.top_candidates );
		...
		std::cout << "  deep_threads=" << state.top_candidates.size() << "  progress_every_seconds=" << deep_progress_sec << "\n\n";
		...
		auto deep_worker = [ & ]( int thread_id ) {
			const LinearBatchTopCandidate& current_candidate = state.top_candidates[ static_cast<std::size_t>( thread_id ) ];
