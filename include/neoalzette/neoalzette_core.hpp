...
namespace TwilightDream
{
	...
	public:
		...
		static constexpr std::uint32_t generate_dynamic_diffusion_mask0_transpose( std::uint32_t x ) noexcept
		{
			return generate_dynamic_diffusion_mask1( x );
		}

		static constexpr std::uint32_t generate_dynamic_diffusion_mask1_transpose( std::uint32_t x ) noexcept
		{
			return generate_dynamic_diffusion_mask0( x );
		}
