...
void NeoAlzetteDifferentialSearch::execute_subround0(
    ...
    // ========================================================================
    // 
    // 问题：给定 (dB, beta)，找所有权重≤weight_budget的dB_after
    // 
    // 方法1：使用pDDT表（最优）
    //   - 如果config提供了pDDT表，直接查询
    //   - 返回所有高概率候选（可能有1000+个）
    //   - 这是论文中"应用"pDDT的方式！
    // 
    // 方法2：启发式枚举（fallback）
    ...
    // 定义回调类型，用于处理每个候选
    // 参数：dB_after（候选输出差分），w1（Step 1的权重）
    auto process_candidate = [&](std::uint32_t dB_after, int w1) {
        // 剪枝1：权重超过预算
        if (w1 < 0 || w1 >= weight_budget) return;
