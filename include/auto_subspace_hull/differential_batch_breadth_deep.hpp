...
namespace TwilightDream::hull_callback_aggregator
{
	...
	struct DifferentialBatchTopCandidate
	{
		...
		TwilightDream::auto_search_differential::SearchWeight entry_round1_weight = TwilightDream::auto_search_differential::INFINITE_WEIGHT;
	...
	static inline std::size_t differential_resolve_batch_top_candidate_limit(
		...
		DifferentialBatchTopCandidate& candidate )
	{
		...
		return
			...
			r.read_u64( candidate.entry_round1_weight ) &&
		...
		DifferentialBatchBreadthDeepOrchestratorResult&		out )
	{
		...
		const std::size_t deep_top_k =
			differential_resolve_batch_top_candidate_limit( jobs.size(), breadth_threads, selection_top_k );
		...
		run_named_worker_group_with_monitor( "differential-batch-breadth", breadth_threads, [ & ]( TwilightDream::runtime_component::RuntimeTaskContext& context ) { worker( int( context.slot_index ) ); }, progress_monitor );
		...
		const std::size_t deep_worker_count = std::min<std::size_t>( top_candidates.size(), std::size_t( breadth_threads ) );
		std::cout << "[Batch][Deep] start: inputs=" << top_candidates.size() << " deep_threads=" << deep_worker_count << " progress_every_seconds=" << deep_progress_sec << "\n";
		...
		auto deep_worker = [ & ]( int thread_id ) {
			for ( ;; )
			{
				...
				const TwilightDream::auto_search_differential::SearchWeight seed_weight = current_candidate.best_weight;
				...
				deep_results[ candidate_index ].job_index = current_candidate.job_index;
				deep_results[ candidate_index ].result = std::move( result );
			}
		};

		run_named_worker_group( "differential-batch-deep", int( deep_worker_count ), [ & ]( TwilightDream::runtime_component::RuntimeTaskContext& context ) { deep_worker( int( context.slot_index ) ); } );
		...
		DifferentialBatchBreadthDeepOrchestratorResult& out )
	{
		...
		const int breadth_threads = std::max( 1, worker_thread_count );
		const std::size_t deep_top_k =
			differential_resolve_batch_top_candidate_limit( state.jobs.size(), breadth_threads, selection_top_k );
		...
		if ( state.stage == DifferentialBatchSelectionCheckpointStage::Breadth )
		{
			...
			run_named_worker_group_with_monitor( "differential-batch-breadth", breadth_threads, [ & ]( TwilightDream::runtime_component::RuntimeTaskContext& context ) { breadth_worker( int( context.slot_index ) ); }, progress_monitor );
			differential_sort_batch_top_candidates( state.top_candidates );
			if ( state.top_candidates.size() > deep_top_k )
				state.top_candidates.resize( deep_top_k );
		...
		const std::size_t deep_worker_count = std::min<std::size_t>( state.top_candidates.size(), std::size_t( breadth_threads ) );
		std::cout << "[Batch][Deep] start: inputs=" << state.top_candidates.size() << " deep_threads=" << deep_worker_count << " progress_every_seconds=" << deep_progress_sec << "\n";
		...
		auto deep_worker = [ & ]( int thread_id ) {
			for ( ;; )
			{
				...
				const DifferentialBatchTopCandidate& current_candidate = state.top_candidates[ candidate_index ];
