#pragma once
#if !defined( TWILIGHTDREAM_LINEAR_HULL_COLLECTOR_CHECKPOINT_HPP )
#define TWILIGHTDREAM_LINEAR_HULL_COLLECTOR_CHECKPOINT_HPP
...
{
	struct LinearHullCollectorCheckpointState
	{
		LinearBestSearchConfiguration base_configuration {};
		LinearHullCollectorExecutionState collector_state {};
		TwilightDream::hull_callback_aggregator::LinearCallbackHullAggregator callback_aggregator {};
	...
	inline std::string default_linear_hull_collector_checkpoint_path(
		int round_count,
		...
		std::uint32_t output_branch_b_mask ) noexcept
	{
		...
		oss << "collector_checkpoint_R" << round_count
			<< "_MaskA" << std::hex << std::setw( 8 ) << std::setfill( '0' ) << output_branch_a_mask
			<< "_MaskB" << std::setw( 8 ) << std::setfill( '0' ) << output_branch_b_mask << std::dec;
		return TwilightDream::runtime_component::make_unique_timestamped_artifact_path( oss.str(), ".ckpt" );
	}

	inline std::string default_linear_hull_collector_runtime_log_path(
		int round_count,
		std::uint32_t output_branch_a_mask,
		std::uint32_t output_branch_b_mask ) noexcept
	{
	...
	namespace linear_hull_collector_checkpoint_detail
	{
			...
			const MatsuiSearchRunLinearResult& result )
		{
			...
			write_bool( w, result.hit_target_best_weight );
			w.write_u32( result.best_input_branch_a_mask );
			w.write_u32( result.best_input_branch_b_mask );
			write_counted_container(
				w,
				result.best_linear_trail,
			...
			MatsuiSearchRunLinearResult& result )
		{
			return
				...
				read_bool( r, result.hit_target_best_weight ) &&
				r.read_u32( result.best_input_branch_a_mask ) &&
				r.read_u32( result.best_input_branch_b_mask ) &&
		...
		static inline void write_hull_shell_summary(
			TwilightDream::auto_search_checkpoint::BinaryWriter& w,
			const LinearHullShellSummary& summary )
		{
		...
		static inline bool read_hull_shell_summary(
			TwilightDream::auto_search_checkpoint::BinaryReader& r,
			LinearHullShellSummary& summary )
		{
			return
		...
		static inline void write_aggregation_result(
			TwilightDream::auto_search_checkpoint::BinaryWriter& w,
			const LinearHullAggregationResult& result )
		{
			...
			write_long_double( w, result.aggregate_signed_correlation );
			write_long_double( w, result.aggregate_abs_correlation_mass );
			write_long_double( w, result.strongest_trail_signed_correlation );
			w.write_u32( result.strongest_input_branch_a_mask );
			w.write_u32( result.strongest_input_branch_b_mask );
			write_counted_map(
				w,
				result.shell_summaries,
				[&]( TwilightDream::auto_search_linear::SearchWeight shell_weight ) { w.write_u64( shell_weight ); },
				[&]( const LinearHullShellSummary& summary ) { write_hull_shell_summary( w, summary ); } );
			write_counted_container(
				w,
		...
		static inline bool read_aggregation_result(
			TwilightDream::auto_search_checkpoint::BinaryReader& r,
			LinearHullAggregationResult& result )
		{
			return
				...
				read_long_double( r, result.aggregate_signed_correlation ) &&
				read_long_double( r, result.aggregate_abs_correlation_mass ) &&
				read_long_double( r, result.strongest_trail_signed_correlation ) &&
				r.read_u32( result.strongest_input_branch_a_mask ) &&
				r.read_u32( result.strongest_input_branch_b_mask ) &&
				read_counted_map<std::map<TwilightDream::auto_search_linear::SearchWeight, LinearHullShellSummary>, TwilightDream::auto_search_linear::SearchWeight, LinearHullShellSummary>(
					r,
					result.shell_summaries,
					[&]( TwilightDream::auto_search_linear::SearchWeight& shell_weight ) { return r.read_u64( shell_weight ); },
					[&]( LinearHullShellSummary& summary ) { return read_hull_shell_summary( r, summary ); } ) &&
				read_counted_container(
					r,
					result.strongest_trail,
					[&]( LinearTrailStepRecord& step ) {
	...
	inline bool write_linear_hull_collector_checkpoint(
		const std::string& path,
		const LinearHullCollectorCheckpointState& state )
	{
		using namespace linear_hull_collector_checkpoint_detail;
		using namespace TwilightDream::hull_batch_checkpoint_shared;
		return TwilightDream::auto_search_checkpoint::write_atomic(
			path,
			[&]( TwilightDream::auto_search_checkpoint::BinaryWriter& w ) {
				if ( !TwilightDream::auto_search_checkpoint::write_header( w, TwilightDream::auto_search_checkpoint::SearchKind::LinearResidualFrontierCollector ) )
					return false;
				write_config( w, state.base_configuration );
				w.write_u32( state.output_branch_a_mask );
				w.write_u32( state.output_branch_b_mask );
				...
				write_trail_vector( w, state.collector_state.context.current_linear_trail );
				...
				TwilightDream::hull_callback_aggregator::write_linear_callback_hull_aggregator( w, state.callback_aggregator );
				return w.ok();
			} );
	}

	inline bool read_linear_hull_collector_checkpoint(
		const std::string& path,
		LinearHullCollectorCheckpointState& state )
	{
		using namespace linear_hull_collector_checkpoint_detail;
		using namespace TwilightDream::hull_batch_checkpoint_shared;
		...
		if ( !TwilightDream::auto_search_checkpoint::read_header( r, kind ) )
			return false;
		if ( kind != TwilightDream::auto_search_checkpoint::SearchKind::LinearResidualFrontierCollector )
			return false;
		if ( !read_config( r, state.base_configuration ) )
			return false;
		if ( !r.read_u32( state.output_branch_a_mask ) )
			return false;
		if ( !r.read_u32( state.output_branch_b_mask ) )
			return false;
		...
		if ( !r.read_string( state.runtime_log_path ) )
			return false;
		if ( !read_trail_vector( r, state.collector_state.context.current_linear_trail ) )
			return false;
		...
		if ( !read_aggregation_result( r, state.collector_state.aggregation_result ) )
			return false;
		if ( !TwilightDream::hull_callback_aggregator::read_linear_callback_hull_aggregator( r, state.callback_aggregator ) )
			return false;
		...
		state.collector_state.context.start_output_branch_a_mask = state.output_branch_a_mask;
		state.collector_state.context.start_output_branch_b_mask = state.output_branch_b_mask;
		state.collector_state.context.best_weight = successor_search_weight( state.collector_state.collect_weight_cap );
		...
		state.collector_state.context.current_linear_trail.reserve( std::size_t( std::max( 1, state.base_configuration.round_count ) ) );
