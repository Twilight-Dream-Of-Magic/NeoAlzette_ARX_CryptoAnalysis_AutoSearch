...
{
	enum class OuterSourceNamespace : std::uint8_t
	{
		WrapperFixedSource = 0,
		BatchJob = 1,
		SubspaceJob = 2
	};

	static inline const char* outer_source_namespace_to_string( OuterSourceNamespace value ) noexcept
	{
		switch ( value )
		{
		case OuterSourceNamespace::WrapperFixedSource:
			return "wrapper_fixed_source";
		case OuterSourceNamespace::BatchJob:
			return "batch_job";
		case OuterSourceNamespace::SubspaceJob:
			return "subspace_job";
		default:
			return "unknown";
		}
	...
	static inline OuterGrowthDriverResult<Row, BestResult, RejectionReason> run_outer_growth_driver(
		...
		std::uint64_t growth_shell_start,
		...
		RowCumulativeAbsFn&& row_cumulative_abs )
	{
		...
		std::uint64_t base_shell_weight = growth_shell_start;
		...
		{
			const std::uint64_t shell_weight = base_shell_weight + shell_index;
