...
namespace TwilightDream::hull_callback_aggregator
{
	...
	struct DifferentialCollectedTrailRecord
	{
		...
		TwilightDream::auto_search_differential::SearchWeight total_weight = 0;
	...
	struct DifferentialEndpointHullSummary
	{
		...
		long double											 aggregate_probability = 0.0L;
		long double											 strongest_trail_probability = 0.0L;
		std::map<TwilightDream::auto_search_differential::SearchWeight, DifferentialEndpointShellSummary> shell_summaries {};
		std::vector<TwilightDream::auto_search_differential::DifferentialTrailStepRecord> strongest_trail {};
	};
	...
	struct DifferentialSourceHullSummary
	{
		...
		std::map<TwilightDream::auto_search_differential::SearchWeight, DifferentialCallbackShellSummary> shell_summaries {};
		std::vector<TwilightDream::auto_search_differential::DifferentialTrailStepRecord> strongest_trail {};
	};
	...
	struct DifferentialCallbackHullAggregator
	{
		...
		const DifferentialCallbackShellSummary* find_shell_summary( TwilightDream::auto_search_differential::SearchWeight shell_weight ) const noexcept
		{
		...
		static bool is_selected_shell( TwilightDream::auto_search_differential::SearchWeight shell_weight, TwilightDream::auto_search_differential::SearchWeight shell_start, std::uint64_t shell_count ) noexcept
		{
		...
		void for_each_selected_shell( TwilightDream::auto_search_differential::SearchWeight shell_start, std::uint64_t shell_count, Fn&& fn ) const
		{
		...
		{
			auto& source = source_hulls[ DifferentialHullSourceKey { source_context.source_index, source_context.source_namespace } ];
		...
		static bool shell_is_selected( TwilightDream::auto_search_differential::SearchWeight shell_weight, TwilightDream::auto_search_differential::SearchWeight shell_start, std::uint64_t shell_count ) noexcept
		{
			if ( shell_start >= TwilightDream::auto_search_differential::INFINITE_WEIGHT )
				return true;
	...
	static inline long double compute_differential_source_total_probability_theorem( const DifferentialSourceHullSummary& source_summary ) noexcept
	{
		(void)source_summary;
		return 1.0L;
	}

	static inline long double compute_differential_callback_hull_source_union_total_probability_theorem( const DifferentialCallbackHullAggregator& callback_aggregator ) noexcept
	{
		long double total_probability_theorem = 0.0L;
		for ( const auto& [ source_key, source_summary ] : callback_aggregator.source_hulls )
		{
			(void)source_key;
			total_probability_theorem += compute_differential_source_total_probability_theorem( source_summary );
		}
		return total_probability_theorem;
	}

	struct DifferentialGrowthShellRow
	{
		TwilightDream::auto_search_differential::SearchWeight shell_weight = 0;
		std::uint64_t trail_count = 0;
	...
	static inline DifferentialGrowthShellRow make_differential_growth_shell_row(
		TwilightDream::auto_search_differential::SearchWeight shell_weight,
	...
	static inline bool write_differential_shell_summary_csv(
		...
		TwilightDream::auto_search_differential::SearchWeight shell_start,
...
#include "auto_subspace_hull/differential_batch_breadth_deep.hpp"
