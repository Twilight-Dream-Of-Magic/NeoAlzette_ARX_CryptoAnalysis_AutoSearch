...
static bool write_linear_subspace_coverage_report(
		...
		const GeneratedSubspaceEvidenceSummary& generated_evidence )
	{
		...
		out << "kind=linear_subspace_coverage\n";
	...
	static bool read_linear_support_key_value_file( const std::string& path, std::map<std::string, std::string>& kv )
	{
	...
	static bool parse_linear_support_u64_string( const std::string& text, std::uint64_t& value )
	{
		return parse_unsigned_integer_64( text.c_str(), value );
	}

	static bool parse_linear_support_weight_string( const std::string& text, SearchWeight& value )
	{
		if ( text == "-1" )
		{
		...
		std::uint64_t raw = 0;
		if ( !parse_linear_support_u64_string( text, raw ) )
			return false;
		value = static_cast<SearchWeight>( raw );
		return true;
	}

	struct LinearEvidenceOnlyReportRecord
	{
	...
	static bool read_linear_evidence_only_report(
		const std::string& path,
		LinearEvidenceOnlyReportRecord& report )
	{
		std::map<std::string, std::string> kv {};
		if ( !read_linear_support_key_value_file( path, kv ) )
			return false;
		if ( kv[ "kind" ] != "linear_subspace_evidence_only_report" )
		{
		...
		if ( !parse_linear_support_u64_string( kv[ "subspace_partition_count" ], report.subspace_partition_count ) ) return false;
		if ( !parse_linear_support_u64_string( kv[ "subspace_partition_index" ], report.subspace_partition_index ) ) return false;
		if ( !parse_linear_support_u64_string( kv[ "prescribed_source_count" ], report.prescribed_source_count ) ) return false;
		if ( !parse_linear_support_u64_string( kv[ "active_partition_source_count" ], report.active_partition_source_count ) ) return false;
		report.generated_evidence_available = ( kv[ "generated_evidence_available" ] == "1" );
		if ( !parse_linear_support_weight_string( kv[ "generated_evidence_min_best_weight" ], report.generated_evidence_min_best_weight ) ) return false;
		return true;
	}
	...
	static int run_linear_partition_campaign( const CommandLineOptions& options )
	{
		...
		std::cout << "[Campaign] mode=linear_partition_campaign\n";
		...
		for ( std::uint64_t shard_index = 0; shard_index < options.campaign_partition_count; ++shard_index )
		{
			...
			if ( options.campaign_adaptive_mode )
			{
				...
				const int probe_rc = run_linear_subspace_evidence_only_mode( probe_options );
				if ( probe_rc != 0 )
					return probe_rc;

				LinearEvidenceOnlyReportRecord probe_report {};
				if ( !read_linear_evidence_only_report( probe_options.subspace_coverage_out_path, probe_report ) )
					return 1;
				...
				const int hull_rc = run_linear_subspace_mode( shard_options );
				if ( hull_rc != 0 )
					return hull_rc;
			...
			const int rc = run_linear_subspace_mode( shard_options );
			if ( rc != 0 )
				return rc;
		}
		if ( options.campaign_adaptive_mode )
	...
	static bool write_linear_subspace_evidence_only_report(
		const std::string& path,
		const CommandLineOptions& options,
		const std::string& source_file_path,
		...
		const GeneratedSubspaceEvidenceSummary& generated_evidence )
	{
		...
		out << "kind=linear_subspace_evidence_only_report\n";
		out << "source_file=" << source_file_path << "\n";
		out << "subspace_partition_count=" << options.subspace_partition_count << "\n";
		out << "subspace_partition_index=" << options.subspace_partition_index << "\n";
	...
	static int run_linear_subspace_evidence_only_mode( const CommandLineOptions& options )
	{
		...
		if ( jobs.empty() )
		{
			std::cout << "[SubspaceEvidence] mode=linear_prescribed_subset_certificate\n";
		...
		std::cout << "  note=these certificates still come from the current search-frame linear model; the operator-layer Gap B optimal-mask APIs are not yet wired here as a reduction theorem\n";
		...
		LinearBatchHullRunSummary synthetic_batch_summary {};
		...
		{
			if ( !write_linear_subspace_evidence_only_report( options.subspace_coverage_out_path, options, source_file_path, prescribed_source_count, static_cast<std::uint64_t>( jobs.size() ), generated_evidence ) )
				return 1;
		}

		std::cout << "[SubspaceEvidence][Linear] source_count=" << jobs.size()
