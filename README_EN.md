...
- **Root research / experiment programs** (`test_*.cpp`): differential trail tracing, residual-frontier best-search, and strict-hull wrappers.
- **PNB / neutral-bit tooling**: currently not included in this repository. It is under active development in a separate repository and will not appear here for now.

## Documentation map

- **This README**: user-facing build / run / CLI guide.
- **Architecture / runtime / checkpoint layout**: `AUTO_SEARCH_FRAME_ARCHITECTURE_EN.md`
- **Strict mathematical BNB blueprint**: `AUTO_SEARCH_FRAME_BNB_MATHEMATICAL_BLUEPRINT_CN.md`

For current search semantics, treat the mathematical blueprint as the strict top-level contract and the architecture document as the code-layout / runtime-contract companion.
...
- **Residual-frontier best-search core**
  - `include/auto_search_frame/detail/`
  - `src/auto_search_frame/`
- **Strict-hull / wrapper / collector layer**
  - `include/auto_subspace_hull/`
  - `src/auto_subspace_hull/`
- **PNB / neutral-bit tooling**
  - currently under active development in a separate repository; not included here
...
- PNB / neutral-bit tooling is currently not built from this repository and is being developed in a separate repository.
...
- Best-search core now lives under:
  - `include/auto_search_frame/detail/`
  - `src/auto_search_frame/`
- Strict-hull collector / wrapper support now lives under:
  - `include/auto_subspace_hull/`
  - `src/auto_subspace_hull/`
...
### Windows direct compiler invocation (advanced / optional)

For current direct compiler source lists, prefer:

- `build_and_test.bat` for the authoritative Windows command list
- `CMakeLists.txt` for the authoritative target/source mapping

The direct command lines for best-search and hull-wrapper targets are intentionally not duplicated here anymore, because the residual-frontier core, BNB-detail sources, and `auto_subspace_hull` collector layer share a long source list that evolves with the codebase.

The simple trace target still has a short standalone form:
...
### PNB / neutral-bit tooling status

PNB / neutral-bit tooling is currently **not included in this repository**.

The previous PNB workspace is not available here at the moment. That line of
work is being rebuilt and continued in a **separate repository**.

So for the current repository:

- there is no `pnb_distinguisher/` folder,
- there are no root-level PNB build targets,
- and this repository should be read as the NeoAlzette core + ARX analysis +
  residual-frontier search / hull codebase only.
