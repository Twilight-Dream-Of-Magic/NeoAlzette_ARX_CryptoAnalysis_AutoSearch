...
namespace
{
	...
	struct CommandLineOptions
	{
		...
		SearchWeight	  auto_target_best_weight = INFINITE_WEIGHT;									   // if finite: stop early once best_weight <= target
		...
		bool					  resume_mode ) noexcept
	{
		return evaluate_memory_gate(
			...
			estimate_linear_must_live_bytes( command_line_options, auto_mode, resume_mode ),
	...
	static void print_usage_information( const char* executable_name )
	{
		...
		std::cout << "Usage:\n"
				  ...
				  << "  --enable-z-shell                Enable the z-shell based Weight-Sliced cLAT accelerator for var-var row-side Q2.\n"
				  << "                                   With --z-shell-max-candidates 0, this is a strict-safe exact shell index: shell-by-shell exact,\n"
				  << "                                   and a table miss means only cache/index miss, not impossible / zero-correlation.\n"
				  ...
				  << "                                   Nonzero truncates the materialized helper list; miss still means cache miss only,\n"
				  << "                                   but shell-by-shell exactness is no longer claimed.\n"
				  << "                                   Strict mode forces this back to 0 because nonzero caps are non-strict.\n"
				  << "  --linear-varvar-bnb MODE         Var-var modular-add BnB: fixed_u_enumerate_vw | fixed_vw_column_optimal_u\n"
				  ...
				  << "  --linear-varconst-sub-bnb MODE   Var-const subtraction outer policy:\n"
				  << "                                   fixed_beta_enumerate_alpha | fixed_beta_column_optimal_alpha |\n"
				  << "                                   fixed_alpha_enumerate_beta | fixed_alpha_column_optimal_beta\n"
				  << "                                   fixed_beta_* is the current reverse-stage primary polarity.\n"
				  << "                                   fixed_alpha_* uses the accepted fixed-alpha Q2 semantics once alpha is fixed\n"
				  << "                                   on the branch ^(or when both sides are fixed^), instead of silently treating the\n"
				  << "                                   request as fixed_beta_*.\n"
	...
	static bool parse_command_line_detail_mode( int argument_count, char** argument_values, int start_index, CommandLineOptions& command_line_options )
	{
		...
		for ( ; argument_index < argument_count; ++argument_index )
		{
			...
			{
				std::uint64_t w = 0;
				if ( !parse_unsigned_integer_64( argument_values[ ++argument_index ], w ) )
					return false;
				command_line_options.search_configuration.target_best_weight = static_cast<SearchWeight>( w );
			...
			else if ( argument == "--linear-varvar-bnb" && argument_index + 1 < argument_count )
			{
				const char* const mode = argument_values[ ++argument_index ];
				if ( std::strcmp( mode, "fixed_u_enumerate_vw" ) == 0 )
					command_line_options.search_configuration.varvar_modular_add_bnb_mode =
						LinearVarVarModularAddBnBMode::FixedOutputMaskU_EnumerateInputVW;
				else if ( std::strcmp( mode, "fixed_vw_column_optimal_u" ) == 0 )
					command_line_options.search_configuration.varvar_modular_add_bnb_mode =
			...
			else if ( argument == "--linear-varconst-sub-bnb" && argument_index + 1 < argument_count )
			{
				const char* const mode = argument_values[ ++argument_index ];
				if ( std::strcmp( mode, "fixed_beta_enumerate_alpha" ) == 0 )
				{
					command_line_options.search_configuration.varconst_sub_bnb_mode =
						LinearVarConstSubBnbMode::FixedOutputMaskBeta_EnumerateInputAlpha;
					command_line_options.search_configuration.enable_fixed_beta_outer_hot_path_gate = false;
					command_line_options.search_configuration.enable_fixed_alpha_outer_hot_path_gate = false;
				}
				else if ( std::strcmp( mode, "fixed_beta_column_optimal_alpha" ) == 0 )
				{
					command_line_options.search_configuration.varconst_sub_bnb_mode =
						LinearVarConstSubBnbMode::FixedOutputMaskBeta_ColumnOptimalInputAlpha;
					command_line_options.search_configuration.enable_fixed_beta_outer_hot_path_gate = true;
					command_line_options.search_configuration.enable_fixed_alpha_outer_hot_path_gate = false;
				}
				else if ( std::strcmp( mode, "fixed_alpha_enumerate_beta" ) == 0 )
				{
					command_line_options.search_configuration.varconst_sub_bnb_mode =
				...
				else if ( std::strcmp( mode, "fixed_alpha_column_optimal_beta" ) == 0 )
				{
					command_line_options.search_configuration.varconst_sub_bnb_mode =
						LinearVarConstSubBnbMode::FixedInputMaskAlpha_ColumnOptimalOutputBeta;
					command_line_options.search_configuration.enable_fixed_beta_outer_hot_path_gate = false;
					command_line_options.search_configuration.enable_fixed_alpha_outer_hot_path_gate = true;
				}
				else
					return false;
				command_line_options.search_configuration_was_provided = true;
			...
			else if ( ( argument == "--addition-weight-cap" || argument == "--add-weight-cap" || argument == "--add" ) && argument_index + 1 < argument_count )
			{
				...
				if ( !parse_unsigned_integer_64( argument_values[ ++argument_index ], v ) )
					return false;
				command_line_options.search_configuration.addition_weight_cap = std::min<SearchWeight>( v, 31 );
				command_line_options.search_configuration_was_provided = true;
			}
			else if ( ( argument == "--constant-subtraction-weight-cap" || argument == "--subconst-weight-cap" || argument == "--subconst-cap" || argument == "--subcap" || argument == "--subtract" ) && argument_index + 1 < argument_count )
			{
				...
				if ( !parse_unsigned_integer_64( argument_values[ ++argument_index ], v ) )
					return false;
				command_line_options.search_configuration.constant_subtraction_weight_cap = std::min<SearchWeight>( v, 32 );
				command_line_options.search_configuration_was_provided = true;
			}
		...
		if ( !command_line_options.resume_was_provided )
		{
			if ( !command_line_options.output_masks_were_provided )
			{
				if ( !command_line_options.input_seed_was_provided )
					return false;
				std::mt19937_64 rng( command_line_options.input_seed );
				std::uint32_t generated_mask_a = 0;
				std::uint32_t generated_mask_b = 0;
				do
				{
					generated_mask_a = static_cast<std::uint32_t>( rng() );
					generated_mask_b = static_cast<std::uint32_t>( rng() );
				} while ( generated_mask_a == 0u && generated_mask_b == 0u );
				command_line_options.output_branch_a_mask = generated_mask_a;
				command_line_options.output_branch_b_mask = generated_mask_b;
	...
	static bool parse_command_line_strategy_mode( int argument_count, char** argument_values, int start_index, CommandLineOptions& command_line_options )
	{
		...
		for ( int argument_index = start_index; argument_index < argument_count; ++argument_index )
		{
			...
			{
				std::cerr << "[Batch] ERROR: batch mode is only supported by test_neoalzette_linear_hull_wrapper.exe; use auto mode here.\n";
				return false;
			}
	...
	static bool parse_command_line_auto_mode( int argument_count, char** argument_values, int start_index, CommandLineOptions& command_line_options )
	{
		...
		for ( ; argument_index < argument_count; ++argument_index )
		{
			...
			else if ( argument == "--auto-target-best-weight" && argument_index + 1 < argument_count )
			{
				...
				if ( !parse_unsigned_integer_64( argument_values[ ++argument_index ], w ) )
					return false;
				command_line_options.auto_target_best_weight = static_cast<SearchWeight>( w );
				command_line_options.search_configuration_was_provided = true;
			}
			else if ( argument == "--search-mode" && argument_index + 1 < argument_count )
			{
	...
	static inline double weight_to_abs_correlation( SearchWeight weight )
	{
	...
	{
		return linear_configuration_has_strict_branch_cap( c ) ? SearchMode::Fast : SearchMode::Strict;
	}
	...
	static int run_auto_mode( const CommandLineOptions& command_line_options )
	{
		...
		const MemoryGateEvaluation memory_gate = evaluate_linear_memory_gate( command_line_options, mem, true, false );
		...
		run_named_worker_group_with_monitor( "linear-auto-breadth", breadth_threads_clamped, [ & ]( TwilightDream::runtime_component::RuntimeTaskContext& context ) { worker( int( context.slot_index ) ); }, progress_monitor );
		if ( auto_pipeline_checkpoint.enabled() )
		...
		try
		{
			const SearchWeight						  seed_weight = selected.best_weight;
	...
	static int run_linear_auto_resume_mode( const CommandLineOptions& command_line_options )
	{
		...
		const MemoryGateEvaluation memory_gate = evaluate_linear_memory_gate( command_line_options, mem, true, true );
		const bool strict_search_mode =
			( command_line_options.search_mode_was_provided ? command_line_options.search_configuration.search_mode : infer_search_mode_from_caps( command_line_options.search_configuration ) ) ==
			SearchMode::Strict;
	...
	static int run_resume_mode( const CommandLineOptions& command_line_options )
	{
		...
		if ( checkpoint_kind != TwilightDream::auto_search_checkpoint::SearchKind::LinearResidualFrontierBest )
		{
		...
		const MemoryGateEvaluation memory_gate = evaluate_linear_memory_gate( command_line_options, mem, false, true );
		const bool strict_search_mode =
			( command_line_options.search_mode_was_provided ? command_line_options.search_configuration.search_mode : infer_search_mode_from_caps( command_line_options.search_configuration ) ) ==
			SearchMode::Strict;
	...
	static int run_single_mode( const CommandLineOptions& command_line_options )
	{
		...
		if ( avail_bytes != 0 && headroom_bytes > avail_bytes )
			headroom_bytes = avail_bytes;
		const MemoryGateEvaluation memory_gate = evaluate_linear_memory_gate( command_line_options, mem, false, false );
		const bool strict_search_mode =
			( command_line_options.search_mode_was_provided ? command_line_options.search_configuration.search_mode : infer_search_mode_from_caps( command_line_options.search_configuration ) ) ==
			SearchMode::Strict;
		...
		if ( command_line_options.output_masks_were_generated && command_line_options.input_seed_was_provided )
			std::cout << " (generated from --seed=0x" << std::hex << command_line_options.input_seed << std::dec << ")";
		std::cout << "\n";
		...
		{
			std::cout << "[Strict] enabled: candidate caps forced to 0 (max_injection_input_masks,max_round_predecessors,weight_sliced_clat_max_candidates)\n";
		...
		if ( command_line_options.frontend_mode == CommandLineOptions::FrontendMode::Strategy )
		{
			...
			const auto row_q2_contract = linear_varvar_row_q2_contract( search_configuration );
			std::cout << "  varvar_row_q2_contract=" << linear_varvar_row_q2_contract_kind_to_string( row_q2_contract.kind )
					  << "  shell_by_shell_exact=" << ( row_q2_contract.shell_by_shell_exact ? "yes" : "no" )
					  << "  miss_semantics=" << ( row_q2_contract.miss_means_cache_only_not_impossible ? "cache_only_not_impossible" : "not_applicable" )
					  << "  q2_role=" << ( row_q2_contract.used_as_candidate_source_for_column_mode ? "candidate_source_for_column_mode" : "active_row_side_q2" ) << "\n\n";
			print_linear_varconst_stage_contract_banner( search_configuration );
			std::cout << "\n";
		}
		else
		{
			...
			const auto row_q2_contract = linear_varvar_row_q2_contract( search_configuration );
			std::cout << "[Config] varvar_row_q2_contract=" << linear_varvar_row_q2_contract_kind_to_string( row_q2_contract.kind )
					  << "  shell_by_shell_exact=" << ( row_q2_contract.shell_by_shell_exact ? "yes" : "no" )
					  << "  miss_semantics=" << ( row_q2_contract.miss_means_cache_only_not_impossible ? "cache_only_not_impossible" : "not_applicable" )
					  << "  q2_role=" << ( row_q2_contract.used_as_candidate_source_for_column_mode ? "candidate_source_for_column_mode" : "active_row_side_q2" ) << "\n\n";
			print_linear_varconst_stage_contract_banner( search_configuration );
