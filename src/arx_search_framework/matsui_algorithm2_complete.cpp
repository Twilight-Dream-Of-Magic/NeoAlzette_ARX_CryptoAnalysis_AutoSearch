...
double MatsuiAlgorithm2Complete::compute_xdp_add(
    ...
    int weight = neoalz::arx_operators::xdp_add_lm2001(alpha, beta, gamma);
    if (weight < 0 || weight > 1024) return 0.0;
    return std::ldexp(1.0, -weight); // 2^{-weight}
}
...
MatsuiAlgorithm2Complete::find_max_probability(
    ...
    auto [best_gamma, best_weight] = neoalz::arx_operators::find_optimal_gamma_with_weight(alpha, beta, n);
    if (best_weight < 0 || best_weight > 1024) return {best_gamma, 0.0};
    return {best_gamma, std::ldexp(1.0, -best_weight)};
}
