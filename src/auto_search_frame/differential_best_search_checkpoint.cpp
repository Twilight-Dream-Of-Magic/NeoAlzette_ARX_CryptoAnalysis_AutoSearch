...
namespace TwilightDream::auto_search_differential
{
	...
	namespace
	{
		...
		void mix_injection_transition( TwilightDream::best_search_shared_core::CheckpointFingerprintBuilder& fp, const InjectionAffineTransition& transition ) noexcept
		{
			...
			for ( const std::uint32_t basis : transition.basis_vectors )
				fp.mix_u32( basis );
			fp.mix_i32( transition.rank_weight );
		}
			...
			const ModularAdditionEnumerator& enumerator ) noexcept
		{
			...
			summary.modular_add_shell_cache_entries += static_cast<std::uint64_t>( enumerator.shell_cache.size() );
			...
			fp.mix_i32( enumerator.weight_cap );
			fp.mix_i32( enumerator.target_weight );
			...
			TwilightDream::best_search_shared_core::CheckpointFingerprintBuilder shell_fp {};
			shell_fp.mix_u64( static_cast<std::uint64_t>( enumerator.shell_cache.size() ) );
			for ( const std::uint32_t gamma : enumerator.shell_cache )
			{
				fp.mix_u32( gamma );
				shell_fp.mix_u32( gamma );
			}
			summary.modular_add_shell_cache_hash ^= shell_fp.finish();
			fp.mix_u64( shell_fp.finish() );
		}
		...
		void mix_differential_round_state( TwilightDream::best_search_shared_core::CheckpointFingerprintBuilder& fp, const DifferentialRoundSearchState& state ) noexcept
		{
			...
			fp.mix_i32( state.optimal_weight_first_addition );
			fp.mix_i32( state.weight_cap_first_addition );
			fp.mix_u32( state.output_branch_b_difference_after_first_addition );
			fp.mix_i32( state.weight_first_addition );
			fp.mix_i32( state.accumulated_weight_after_first_addition );
			fp.mix_u32( state.output_branch_a_difference_after_first_constant_subtraction );
			fp.mix_i32( state.weight_first_constant_subtraction );
			fp.mix_i32( state.accumulated_weight_after_first_constant_subtraction );
			...
			fp.mix_i32( state.accumulated_weight_before_second_addition );
			...
			fp.mix_i32( state.optimal_weight_second_addition );
			fp.mix_i32( state.weight_cap_second_addition );
			fp.mix_u32( state.output_branch_a_difference_after_second_addition );
			fp.mix_i32( state.weight_second_addition );
			fp.mix_i32( state.accumulated_weight_after_second_addition );
			...
			fp.mix_i32( state.accumulated_weight_after_second_constant_subtraction );
			...
			fp.mix_i32( state.accumulated_weight_at_round_end );
	...
	bool write_differential_checkpoint( const std::string& path, const DifferentialBestSearchContext& context, const DifferentialSearchCursor& cursor, std::uint64_t elapsed_usec )
	{
		return TwilightDream::auto_search_checkpoint::write_atomic( path, [ & ]( TwilightDream::auto_search_checkpoint::BinaryWriter& w ) {
			if ( !TwilightDream::auto_search_checkpoint::write_header( w, TwilightDream::auto_search_checkpoint::SearchKind::DifferentialBest ) )
				return false;
	...
	bool read_differential_checkpoint( const std::string& path, DifferentialCheckpointLoadResult& out, TwilightDream::runtime_component::BestWeightMemoizationByDepth<std::uint64_t, int>& memo )
	{
		...
		if ( !TwilightDream::auto_search_checkpoint::read_header( r, kind ) )
			return false;
		if ( kind != TwilightDream::auto_search_checkpoint::SearchKind::DifferentialBest )
			return false;
