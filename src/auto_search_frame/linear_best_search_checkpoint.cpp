...
namespace TwilightDream::auto_search_linear
{
	...
	namespace
	{
			...
			const char* end_label )
		{
			...
			for ( const auto& s : trail )
			{
				out << "round_index=" << s.round_index
					...
					<< " branch_a_mask_after_undoing_injection_from_branch_a=" << hex8( s.branch_a_mask_after_undoing_injection_from_branch_a )
					<< " branch_b_mask_before_undoing_injection_from_branch_b=" << hex8( s.branch_b_mask_before_undoing_injection_from_branch_b )
					<< " branch_b_mask_after_undoing_injection_from_branch_b=" << hex8( s.branch_b_mask_after_undoing_injection_from_branch_b )
		...
		void mix_linear_trail_step( TwilightDream::best_search_shared_core::CheckpointFingerprintBuilder& fp, const LinearTrailStepRecord& step ) noexcept
		{
			...
			fp.mix_u32( step.branch_a_mask_after_undoing_injection_from_branch_a );
			fp.mix_u32( step.branch_b_mask_before_undoing_injection_from_branch_b );
			fp.mix_u32( step.branch_b_mask_after_undoing_injection_from_branch_b );
		...
		void mix_subconst_candidate( TwilightDream::best_search_shared_core::CheckpointFingerprintBuilder& fp, const SubConstCandidate& candidate ) noexcept
		{
			...
			fp.mix_u64( candidate.linear_weight );
		}
		...
		void mix_subconst_streaming_cursor( TwilightDream::best_search_shared_core::CheckpointFingerprintBuilder& fp, const SubConstStreamingCursor& cursor ) noexcept
		{
			...
			fp.mix_u64( cursor.weight_cap );
			fp.mix_i32( cursor.nbits );
			fp.mix_u64( cursor.min_abs );
			fp.mix_u64( cursor.emitted_candidate_count );
			...
			fp.mix_u64( static_cast<std::uint64_t>( cursor.heap.size() ) );
			for ( const auto& node : cursor.heap )
			{
				fp.mix_i32( node.bit_index );
				fp.mix_u32( node.prefix );
				const auto dir_x_bits = node.dir.x.bit_pattern();
				const auto dir_y_bits = node.dir.y.bit_pattern();
				fp.mix_u64( dir_x_bits.low64() );
				fp.mix_u64( dir_x_bits.high64() );
				fp.mix_u64( dir_y_bits.low64() );
				fp.mix_u64( dir_y_bits.high64() );
				fp.mix_u64( static_cast<std::uint64_t>( node.scale ) );
				fp.mix_u64( static_cast<std::uint64_t>( node.scale >> 64 ) );
				fp.mix_u64( node.optimistic_abs_weight_packed );
				fp.mix_bool( node.optimistic_abs_weight_is_exact_2pow64 );
				fp.mix_u64( node.optimistic_full_mask );
			}
		...
		void mix_add_streaming_cursor( TwilightDream::best_search_shared_core::CheckpointFingerprintBuilder& fp, const AddVarVarSplit8Enumerator32::StreamingCursor& cursor ) noexcept
		{
			...
			fp.mix_u64( cursor.weight_cap );
			fp.mix_u64( cursor.next_target_weight );
			...
			for ( const auto& frame : cursor.stack )
			{
				...
				fp.mix_u64( frame.remaining_weight );
				fp.mix_u64( static_cast<std::uint64_t>( frame.option_index ) );
				fp.mix_u64( frame.target_weight );
			}
		}
		...
		{
			mix_add_streaming_cursor( fp, cursor );
		}
		...
		void mix_injection_transition( TwilightDream::best_search_shared_core::CheckpointFingerprintBuilder& fp, const InjectionCorrelationTransition& transition ) noexcept
		{
			...
			for ( const std::uint32_t basis : transition.basis_vectors )
				fp.mix_u32( basis );
			fp.mix_u64( transition.rank );
			fp.mix_u64( transition.weight );
		}
			...
			const LinearRoundSearchState& state ) noexcept
		{
			...
			fp.mix_u64( state.round_weight_cap );
			...
			fp.mix_u64( state.second_subconst_weight_cap );
			fp.mix_u64( state.second_add_weight_cap );
			...
			fp.mix_u64( state.base_weight_after_inj_b );
			...
			fp.mix_u64( state.base_weight_after_second_subconst );
			fp.mix_u64( state.first_subconst_weight_cap );
			fp.mix_u64( state.first_add_weight_cap );
			...
			fp.mix_u64( state.weight_first_constant_subtraction_current );
			fp.mix_u32( state.second_add_trail_sum_wire_u );
			fp.mix_u32( state.first_add_trail_sum_wire_u );
	...
	void BestWeightHistory::maybe_write( const LinearBestSearchContext& context, const char* reason )
	{
		...
		out << "varvar_modular_add_bnb_mode=" << linear_varvar_modular_add_bnb_mode_to_string( config.varvar_modular_add_bnb_mode )
			<< "\n";
		out << "varconst_sub_bnb_mode=" << linear_varconst_sub_bnb_mode_to_string( config.varconst_sub_bnb_mode )
			<< "\n";
		out << "enable_fixed_beta_outer_hot_path_gate=" << ( config.enable_fixed_beta_outer_hot_path_gate ? 1 : 0 ) << "\n";
	...
	bool write_linear_checkpoint_payload( TwilightDream::auto_search_checkpoint::BinaryWriter& w, const LinearBestSearchContext& context, const LinearSearchCursor& cursor, std::uint64_t elapsed_usec )
	{
		...
		w.write_u64( context.best_weight );
	...
	bool write_linear_checkpoint( const std::string& path, const LinearBestSearchContext& context, const LinearSearchCursor& cursor, std::uint64_t elapsed_usec )
	{
		return TwilightDream::auto_search_checkpoint::write_atomic( path, [ & ]( TwilightDream::auto_search_checkpoint::BinaryWriter& w ) {
			if ( !TwilightDream::auto_search_checkpoint::write_header( w, TwilightDream::auto_search_checkpoint::SearchKind::LinearResidualFrontierBest ) )
				return false;
	...
	bool read_linear_checkpoint( const std::string& path, LinearCheckpointLoadResult& out, TwilightDream::runtime_component::BestWeightMemoizationByDepth<std::uint64_t, SearchWeight>& memo )
	{
		...
		if ( !TwilightDream::auto_search_checkpoint::read_header( r, kind ) )
			return false;
		if ( kind != TwilightDream::auto_search_checkpoint::SearchKind::LinearResidualFrontierBest )
			return false;
	...
	bool BinaryCheckpointManager::write_now( const LinearBestSearchContext& context, const LinearSearchCursor& cursor, const char* reason )
	{
...

		{
			...
			const std::int64_t best_weight_out = display_search_weight( context.best_weight );
